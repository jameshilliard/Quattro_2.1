...
char * realloc (char *mem, register size_t n);

char *
realloc (char *mem, register size_t n)
{
  register union mhead *p;
...
char * memalign (unsigned int alignment, size_t size);
...
void *
#else
char *valloc (size_t size);

char *
#endif
valloc (size_t size)
{
  return memalign ((unsigned int)getpagesize (), size);
}
#endif /* !HPUX */
...
struct bucket_stats malloc_bucket_stats (int size);
...
struct _malstats malloc_stats (void);
...
void print_malloc_stats (char *s);
