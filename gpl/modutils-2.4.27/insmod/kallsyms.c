...
int KALLSYMS_MAIN (int argc, char **argv)
{
    ...
    {
	    ElfW(Ehdr)	tmp_Ehdr;
	    memcpy(&tmp_Ehdr, &fout->header, sizeof(fout->header));
	    ElfW(swap_elf_hdr)(&tmp_Ehdr);
	    fwrite(&tmp_Ehdr, sizeof(fout->header), 1, stdout);
    }
#else
	...
	{
		ElfW(Shdr)	tmp_shdr;
		memcpy(&tmp_shdr, &fout->sections[i]->header, sizeof(tmp_shdr));
		ElfW(swap_elf_shdr)(&tmp_shdr);
		fwrite(&tmp_shdr, fout->header.e_shentsize, 1, stdout);
	}
#else
	...
	if (fout->sections[i]->header.sh_size) {
		...
		if (strcmp(sec->name, KALLSYMS_SEC_NAME)==0) {
			swap_kallsyms_symbols( sec->contents);
			swap_kallsyms_sections( sec->contents);
			swap_kallsyms_hdr( sec->contents);
		}
	    }
#endif
