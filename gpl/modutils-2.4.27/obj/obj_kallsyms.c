...
#define Elf32_swap_int      	swap_u32

extern unsigned short  swap_u16( unsigned short num);
extern unsigned int swap_u32( unsigned int num);

void
swap_kallsyms_hdr( struct kallsyms_header *khdr)
{
    khdr->size = swap_u32(khdr->size);
    khdr->total_size = ElfW(swap_Word)(khdr->total_size);
    khdr->sections = swap_u32(khdr->sections);
    khdr->section_off = ElfW(swap_Off)(khdr->section_off);
    khdr->section_size = swap_u32(khdr->section_size);
    khdr->symbols = swap_u32(khdr->symbols);
    khdr->symbol_off = ElfW(swap_Off)(khdr->symbol_off);
    khdr->symbol_size = swap_u32(khdr->symbol_size);
    khdr->string_off = ElfW(swap_Off)(khdr->string_off);
    khdr->start = ElfW(swap_Addr)(khdr->start);
    khdr->end = ElfW(swap_Addr)(khdr->end);
}

void
swap_kallsyms_symbol( struct kallsyms_symbol *ksym)
{
    ksym->section_off = ElfW(swap_Off)(ksym->section_off);
    ksym->symbol_addr = ElfW(swap_Addr)(ksym->symbol_addr);
    ksym->name_off = ElfW(swap_Off)(ksym->name_off);
}

void
swap_kallsyms_sections( struct kallsyms_header *k_hdr)
{
    struct kallsyms_section *ksec;
    int	i, nsec;

    nsec = k_hdr->sections;

    ksec = (struct kallsyms_section*) (((char*) k_hdr) + k_hdr->section_off);

    for (i=0; i < nsec ; i++, ksec ++ )
    {
        ksec->start = ElfW(swap_Addr)(ksec->start);
        ksec->size = ElfW(swap_Word)(ksec->size);
        ksec->name_off = ElfW(swap_Off)(ksec->name_off);
        ksec->flags = ElfW(swap_Word)(ksec->flags);
    }
}

void
swap_kallsyms_symbols( struct kallsyms_header *k_hdr)
{
    int i;
