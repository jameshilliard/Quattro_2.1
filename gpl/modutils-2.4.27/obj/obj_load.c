...
#ifdef SWAP_ENDIAN
      ElfW(swap_elf_shdr)(&section_headers[i]);
#endif
...
unsigned short
swap_u16( unsigned short num)
{
	return (((num & 0xFF00) >> 8) | ((num & 0x00FF) << 8));
}

unsigned int
swap_u32( unsigned int num)
{
	return swap_u16( ((num & 0xFFFF0000) >> 16)) | (swap_u16( (num & 0x0000FFFF)) << 16) ;
}

#define Elf32_swap_Half		swap_u16
#define Elf32_swap_Word		swap_u32
#define Elf32_swap_Sword	swap_u32
#define Elf32_swap_Addr		swap_u32
#define Elf32_swap_Off		swap_u32
#define Elf32_swap_Section	swap_u16

#if 0
#define Elf64_swap_Half	swap_u16
#define Elf64_swap_Word	swap_u32
#define Elf64_swap_Off	swap_u32
#endif


void
ElfW(swap_elf_hdr)( ElfW(Ehdr) *pEHdr)
{
	pEHdr->e_type = ElfW(swap_Half)(pEHdr->e_type);
	pEHdr->e_machine = ElfW(swap_Half)(pEHdr->e_machine);
	pEHdr->e_version = ElfW(swap_Word)(pEHdr->e_version);
	pEHdr->e_entry = ElfW(swap_Addr)(pEHdr->e_entry);
	pEHdr->e_phoff = ElfW(swap_Off)(pEHdr->e_phoff);
	pEHdr->e_shoff = ElfW(swap_Off)(pEHdr->e_shoff);
	pEHdr->e_flags = ElfW(swap_Word)(pEHdr->e_flags);
	pEHdr->e_ehsize = ElfW(swap_Half)(pEHdr->e_ehsize);
	pEHdr->e_phentsize = ElfW(swap_Half)(pEHdr->e_phentsize);
	pEHdr->e_phnum = ElfW(swap_Half)(pEHdr->e_phnum);
	pEHdr->e_shentsize = ElfW(swap_Half)(pEHdr->e_shentsize);
	pEHdr->e_shnum = ElfW(swap_Half)(pEHdr->e_shnum);
	pEHdr->e_shstrndx = ElfW(swap_Half)(pEHdr->e_shstrndx);

}

void
ElfW(swap_elf_shdr)( ElfW(Shdr) *pShdr)
{
	pShdr->sh_name = ElfW(swap_Word)(pShdr->sh_name);
	pShdr->sh_type = ElfW(swap_Word)(pShdr->sh_type);
	pShdr->sh_flags = ElfW(swap_Word)(pShdr->sh_flags);
	pShdr->sh_addr = ElfW(swap_Addr)(pShdr->sh_addr);
	pShdr->sh_offset = ElfW(swap_Off)(pShdr->sh_offset);
	pShdr->sh_size = ElfW(swap_Word)(pShdr->sh_size);
	pShdr->sh_link = ElfW(swap_Word)(pShdr->sh_link);
	pShdr->sh_info = ElfW(swap_Word)(pShdr->sh_info);
	pShdr->sh_addralign = ElfW(swap_Word)(pShdr->sh_addralign);
	pShdr->sh_entsize = ElfW(swap_Word)(pShdr->sh_entsize);
}

void
ElfW(swap_elf_sym)( ElfW(Sym) *pSym)
{
	pSym->st_name = ElfW(swap_Word)(pSym->st_name);
	pSym->st_value = ElfW(swap_Addr)(pSym->st_value);
	pSym->st_size = ElfW(swap_Word)(pSym->st_size);
	pSym->st_shndx = ElfW(swap_Section)(pSym->st_shndx);
}

void
ElfW(swap_elf_rela)( ElfW(Rela) *pRela)
{
	pRela->r_offset = ElfW(swap_Addr)(pRela->r_offset);
	pRela->r_info = ElfW(swap_Word)(pRela->r_info);
	pRela->r_addend = ElfW(swap_Sword)(pRela->r_info);
}
