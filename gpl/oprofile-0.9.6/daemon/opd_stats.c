...
void opd_print_stats(void)
{
	...
	print_if("Nr. event lost due to buffer overflow: %u\n",
	       OP_MNTDIR "stats", "event_lost_overflow", 1);
	print_if("Nr. samples lost due to no mapping: %u\n",
	       OP_MNTDIR "stats", "sample_lost_no_mapping", 1);
	print_if("Nr. backtraces skipped due to no file mapping: %u\n",
	       OP_MNTDIR "stats", "bt_lost_no_mapping", 0);
	print_if("Nr. samples lost due to no mm: %u\n",
	       OP_MNTDIR "stats", "sample_lost_no_mm", 1);

	opd_ext_print_stats();

	if (!(dir = opendir(OP_MNTDIR "/stats/")))
		goto out;
	while ((dirent = readdir(dir))) {
		...
		if (sscanf(dirent->d_name, "cpu%d", &cpu_nr) != 1)
			continue;
		snprintf(path, 256, OP_MNTDIR "stats/%s", dirent->d_name);
