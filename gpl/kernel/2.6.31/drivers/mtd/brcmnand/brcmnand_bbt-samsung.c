...
#ifdef CONFIG_TIVO_YUNDO 
		if(writeops==0)
			PRINTK_RNG("[BBM] No need Update BBT\n");
#endif		
...
int brcmnand_smt_isbadBlcok (struct mtd_info *mtd, loff_t offs)
{
	struct brcmnand_chip *this = mtd->priv;
	uint32_t block; // Used as an index, so 32bit.
	uint8_t	res;
...
EXPORT_SYMBOL (brcmnand_smt_isbadBlcok);
