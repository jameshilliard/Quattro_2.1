...
{printk("%s: offset=%0llx, oob=\n", __FUNCTION__, sliceOffset); print_oobbuf(oobarea, 16);}
			...
			if (valid == BRCMNAND_CORRECTABLE_ECC_ERROR) 
			{
				...
				if(offset<0x7f00000)
					printk("[NAND] Get BRCMNAND_CORRECTABLE_ECC_ERROR !(offset=%0llx) (valid %d) (ret %d)\n",offset,valid,ret);
#endif
			}
			else
				ret = 0;
...
#ifdef CONFIG_TIVO_YUNDO 
			if(offset<0x7f00000)			
				printk("[NAND] Get BRCMNAND_UNCORRECTABLE_ECC_ERROR !(offset=%0llx)  (valid %d) (ret %d)\n",offset,valid,ret);			
#endif
			break;
		     ...
		     size_t *retlen, uint8_t *buf)
{
	...
	extern struct smt_bbt_info *gSkipTBL;
	extern int gSkipFlag;
...
printk("-->%s, offset=%0llx, len=%08x\n", __FUNCTION__, from, len);}
	...
	if(gSkipFlag ==1)
	{
		if(gSkipTBL[from>>17].iOffset==USED_SKIP_BLOCK)
		{
			printk("[NAND] There are no remaining blocks !!\n");
			return -EINVAL;
		}
		from+=gSkipTBL[from>>17].iOffset;
	}
