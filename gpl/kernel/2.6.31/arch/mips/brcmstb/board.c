...
struct rng_mtd_partition {
	char name[32];			/* identifier string */
	uint64_t size;			/* partition size */
	uint64_t offset;		/* offset within the master MTD space */
};

#define NUMBER_OF_PART 	6

struct rng_mtd_partition mtd[NUMBER_OF_PART]=
	{
	{"flash0.cfe",               0x80000,     0x0},
	{"flash0.uniqid",            0x10000,     0x80000},
	{"flash0.CM_HDCP_TUNERIF",   0x10000,     0x90000},
	{"flash0.TIVO_KEY_CFE_CFG",  0x10000,     0xA0000},
	{"flash0.PLAT_BL_HEADER",    0x10000,     0xB0000},
	{"flash0.SWDL",	             0x140000,    0xC0000}
	};
#endif	/* CONFIG_TIVO_YUNDO */
...
int __init board_get_partition_map(struct mtd_partition **p)
{
	...
	{
		ret[i].offset = mtd[i].offset;
		ret[i].size = mtd[i].size;
		ret[i].name = mtd[i].name;
	}
