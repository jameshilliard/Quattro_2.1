...
static inline int __init parse_boardname(const char *buf, void *slop)
{
	...
	//printk("CFE BOARD NAME IS %s\n", brcm_cfe_boardname);
	if (strlen(brcm_cfe_boardname) == 0)
	{
	    brcm_cfe_bootloader = 0;
	    //printk("THIS IS A CISCO BOOTLOADER\n");
	}
	else
	{
	    brcm_cfe_bootloader = 1;
	    //printk("THIS IS A CFE BOOTLOADER\n");
	}
	return(0);
}
EXPORT_SYMBOL(brcm_cfe_bootloader);
...
#if 1	// drw: temporary hack to supply values not currently provided by LxLoader

	// these values will cause a "whole device" partition to be created
	brcm_mtd_rootfs_start = 0x0BE00000;
	brcm_mtd_rootfs_len   = 0x04080000;

//	brcm_mtd_flash_size_mb = 512;

	brcm_mtd_flash_type[0] = 'N';
	brcm_mtd_flash_type[1] = 'A';
	brcm_mtd_flash_type[2] = 'N';
	brcm_mtd_flash_type[3] = 'D';
	brcm_mtd_flash_type[4] = 0;
