...
/*=============================================================================
 * UART functions for BCM_LINUX_UARTA_IRQ and BCM_LINUX_UARTB_IRQ interrupts
 *-----------------------------------------------------------------------------
...
/*=============================================================================
 * Performance functions for BCM_PERFCOUNT_IRQ interrupt
 */
#ifdef CONFIG_OPROFILE
#warning "OPROFILE: Including the interrupt descriptor for BCM MIPS PERF" 
...
{		
#ifdef CONFIG_OPROFILE
	/* Dispatch any timer interrupt events */
	do_IRQ(BCM_PERFCOUNT_IRQ, regs);
#endif
	/* Dispatch any timer interrupt events */
...
void brcm_mips_int2_dispatch(struct pt_regs *regs)
{
	...
	//brcm_mips_int2_disable(0); /* Should already be disabled */
...
/* Additions diagnostic functions */
...
/*---------------------------------------------------------------------------*\
	brcm_irq_setup()
\*---------------------------------------------------------------------------*/
/*! @brief Broadcom specific interrupt descriptors initialization.
*/
void __init brcm_irq_setup(void)
{
	...
	printk("timer irq %d end %d",BCM_LINUX_SYSTIMER_IRQ, BCHP_HIF_CPU_INTR1_INTR_W0_STATUS_reserved0_SHIFT+32);
	//INTC->IrqMask = 0UL;
	//INTC->IrqStatus = 0UL;
