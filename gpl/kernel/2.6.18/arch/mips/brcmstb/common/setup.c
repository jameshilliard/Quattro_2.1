...
// Pointer to an EPF handler function which may or may not eventually be
// resolved by some module, if so it needs to be invoked prior to reset. 
void (*EPFHandlerFunction)(void)=NULL;
EXPORT_SYMBOL(EPFHandlerFunction);

// perform a reset by cycling the system power
void tivo_machine_restart(void)
{
    local_irq_disable();                                // we don't want anything to work anymore
    BDEV_UNSET(BCHP_GIO_IODIR_LO, 1<<22);               // assert PS_RESET aka gpio 22
    BDEV_SET(BCHP_GIO_DATA_LO, 1<<22);                  // this will soon cause power supply to cycle
    if (EPFHandlerFunction) (*EPFHandlerFunction)();    // meawhile, maybe invoke EPFHandlerFunction, note it won't return
    printk("Halted.\n");
    while(1);                                           // otherwise spin until dead
}    
EXPORT_SYMBOL(tivo_machine_restart); 
#else
// Neutron, etc, just reset the CPU
static void tivo_machine_restart(void)
{
