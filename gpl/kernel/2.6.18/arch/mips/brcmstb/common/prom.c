 ...
 *
 * Copyright 2011 TiVo Inc. All Rights Reserved. Broadcom Corporation
...
#endif /* !defined(BRCM_MEMORY_STRAPS) */
...
/* For the 97xxx series STB, process CFE boot parms */
...
#elif defined(CONFIG_TIVO_KONTIKI)
        /* Platform specific Init */
...
#ifdef CONFIG_TIVO
      // Remove invalid params on release builds. This is called immediately
      // after obtaining command line from firmware, before kernel has added
      // anything.
      cmdline_whitelist(cfeBootParms);
#endif      
	 ...
	 *      showcet(CONFIG_TIVO_KONTIKI):       10. Display the correctable error count
	 *      resetcet(CONFIG_TIVO_KONTIKI):      11. Reset the correctable error count to 0 and table to all 0xff
	 */
...
#if (defined(CONFIG_MTD_BRCMNAND_CORRECTABLE_ERR_HANDLING) && defined(CONFIG_TIVO_KONTIKI))
...
#endif /* (defined(CONFIG_MTD_BRCMNAND_CORRECTABLE_ERR_HANDLING) && defined(CONFIG_TIVO_KONTIKI)) */
				break;
			}
			c = *(from++);
			if (!c)
...
/*TiVo platform specific setup.c fills in default boot parameters*/
#if !defined(CONFIG_TIVO)
		if (p == NULL || *p == '\0') {
...
noncfe:
#endif
        // perform platform specific init
        tivo_init();
#endif
	...
	// THT: PR21410 Implement memory hole in init_bootmem, now just record the memory size.
	{
		...
		if (ramSizeMB <= 256) {
...
#else //#ifdef CONFIG_TIVO_KONTIKI
			...
			*/
			add_memory_region(0, 256<<20, BOOT_MEM_RAM);
#endif
#endif // #ifdef CONFIG_TIVO_KONTIKI
...
#ifdef CONFIG_TIVO_KONTIKI
/*
 * Thomson needs to read the MAC(s) after determineBootFromFlashOrRom() because
 * we assure that address inversion is off in that function.  Otherwise we could
 * get different results between the BSL and CFE
 */
#define TMM_FLASH_MACADDR_ADDR (0xBFC5F8F0)     // assumes inversion turned on at this point

/* JLF Note:  Doesn't enter this loop because we got a MAC address from CFE */
	if(gNumHwAddrs <= 0) {
		unsigned int i, mac, ok = 0, ok2 = 0;

		mac = TMM_FLASH_MACADDR_ADDR;
		for(i = 0; i < 6; i += 2) {
			u16 word = readw((void* )(mac + i));

			if(word != 0x0000 && word != 0xffff)
				ok = 1;

			gHwAddrs[0][i] = word & 0xff;
			gHwAddrs[0][i + 1] = word >> 8;
		}

		if(!ok || (gHwAddrs[0][1] & 1)) {
			sprintf(msg, "WARNING read invalid MAC address from flash at %08X\n", mac);
		}
		else {
			sprintf(msg, "EMAC_0 address "
						 "%02x:%02x:%02x:%02x:%02x:%02x fetched from addr %08X\n",
						 gHwAddrs[0][0], gHwAddrs[0][1], gHwAddrs[0][2],
						 gHwAddrs[0][3], gHwAddrs[0][4], gHwAddrs[0][5],
						 mac);
		}
		...
		for(i = 0; i < 6; i += 2) {
			...
			gHwAddrs[1][i] = word & 0xff;
			gHwAddrs[1][i + 1] = word >> 8;
		}
		if(ok2) {
			sprintf(msg, "EMAC_1 address "
						 "%02x:%02x:%02x:%02x:%02x:%02x fetched from addr %08X\n",
						 gHwAddrs[1][0], gHwAddrs[1][1], gHwAddrs[1][2],
						 gHwAddrs[1][3], gHwAddrs[1][4], gHwAddrs[1][5],
						 mac);
			uart_puts(msg);
		}
