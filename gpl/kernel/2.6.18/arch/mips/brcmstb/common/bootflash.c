...
#define EBI_CS_BASE_0 *((volatile unsigned long *)(KSEG1ADDR(0x10000000 + BCHP_EBI_CS_BASE_0)))
#endif
...
// If Kontiki, assume Flash
void determineBootFromFlashOrRom(void)
{
   char msg[128];
   unsigned long value;

   // Make sure EBI inversion is on.  This is required
   // by some MIPS cores that need the reset code at
   // BFC00000 to start up SMP.  Also the MAC is located
   // at a consistant location regardless of Flash size.
   // Inversion will be turned off later by the MTD driver.
   value = *(volatile unsigned long*)BCM_PHYS_TO_K1(BCHP_PHYSICAL_OFFSET+BCHP_EBI_CS_CONFIG_0);
   value |= BCHP_EBI_CS_CONFIG_0_mask_en_MASK;
   *(volatile unsigned long*)BCM_PHYS_TO_K1(BCHP_PHYSICAL_OFFSET+BCHP_EBI_CS_CONFIG_0) = value;
   value = *(volatile unsigned long*)BCM_PHYS_TO_K1(BCHP_PHYSICAL_OFFSET+BCHP_EBI_CS_BASE_0);
   RT_PHYS_FLASH_BASE = value & BCHP_EBI_CS_BASE_0_base_addr_MASK;
   sprintf(msg, "**********BOOTEDFROMFLASH, Base=%08lx, Size = 2^%lu\n",
           RT_PHYS_FLASH_BASE, (value & BCHP_EBI_CS_BASE_0_size_MASK) + 13);
   uart_puts(msg);
}

#elif defined(CONFIG_BRCM_SKIP_CHECK_BOOTROM)
void determineBootFromFlashOrRom(void)
{
	char msg[64];
