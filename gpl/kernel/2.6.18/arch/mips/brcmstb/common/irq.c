...
/* Using the global pointer only as a measure to dump 
 * the irq # on the stack frame of brcm_intc_dispatch()
 * before making a call to do_IRQ() -
 */
int *dummy_ptr;
#endif
...
asmlinkage void plat_irq_dispatch(struct pt_regs *regs)
{
	...
	if (pending & STATUSF_IP7)
		...
		if (printk_ratelimit()) {
			printk("spurious_interrupt: pending %x, c0cause %x, c0status %x c0epc %x\n",
				   pending, read_c0_cause(), read_c0_status(), read_c0_epc());
		}
#endif
	}
}
