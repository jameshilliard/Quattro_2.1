...
static int plognocache = 0;
#ifdef MODULE
module_param(plognocache, int, 0);
#else
static int __init plognocache_setup(char *s)
{
    plognocache=simple_strtoul(s,NULL,0);
    return 1;
}
__setup("plognocache=", plognocache_setup);
#endif
...
static int plog_assign_region(void)
{
	...
	if (plognocache == 0)
		...
		s_processLog = (int *)KSEG1ADDR(plog_region_start);
    }
...
void _plog_bh_enter(int num, unsigned long pc)
{
	struct _pl_bhre entry;
	entry.num = num;
	entry.pc = pc;
	_plLogEvent(PLOG_TYPE_BHE, sizeof(entry), &entry);
}
...
void _plog_diskstart_sata(struct request *rq)
{
    int buffer[1];

    /* 
     * 2.4 convention was:
     * 	type 0: non-TiVo request
     * 	type 1: TiVo RT request
     * 	type 2: TiVo non-RT request
     * 2.6 convention: (doesnt distinguish tivo v/s non-Tivo)
     * 	type 0: -- not-used --
     * 	type 1: RT 
     * 	type 2: non-RT 
     */
    if ((rq->deadline == 0) || (rq->deadline == INFINITE_DEADLINE))
        buffer[0] = 2;      // type 2: non-RT request
    else
        buffer[0] = 1;      // type 1: RT request

    _plLogEvent(PLOG_TYPE_DISK_START, sizeof(buffer), buffer);
}

void _plog_diskstop_sata(struct request *rq, int write)
{
    int buffer[3];

    buffer[0] = rq->nr_sectors;

    buffer[1] = write;  // READ 0, WRITE 1
...
#if 0 
 /*
  *XXX: below _functions are replaced with _functions_sata
...
void _plog_pagefault_ent(unsigned long addr, unsigned long pc)
{
	struct _pl_pfl entry;
	entry.pid = current->pid;
	entry.addr = addr;
	entry.pc = pc;
	entry.preempt_count = preempt_count(); 
	_plLogEvent(PLOG_TYPE_PFE, sizeof(entry), &entry);
}

void _plog_flush_cache(int num, unsigned long ra)
{
	struct _pl_flush_c entry;
	entry.num = num;
	entry.ra = ra;	
	_plLogEvent(PLOG_TYPE_FLUSH_CACHE, sizeof(entry), &entry);
}

void _plog_dma_coh(int num, unsigned long addr, unsigned long size, unsigned long ra)
{
	struct _pl_dma_c entry;
	entry.num = num;
	entry.addr = addr;
	entry.size = size;
	entry.ra = ra;
	_plLogEvent(PLOG_TYPE_DMA_CACHE, sizeof(entry), &entry);
}
