...
#ifdef CONFIG_TIVO_WAR_FOR_BUG303720
 #define SOFTWARE_TLB_RANDOM 0
#endif
...
#ifdef CONFIG_TIVO_WAR_FOR_BUG303720
#if SOFTWARE_TLB_RANDOM
static __init void build_software_random(u32 **p, unsigned int k0,
                                         unsigned int k1)
{
   i_addiu(p, k0, 0, 31);
   i_mfc0(p, k1, C0_WIRED);
   i_subu(p, k0, k0, k1);
   i_mfc0(p, k1, C0_COUNT);
   i_and(p, k1, k1, k0);
   i_mfc0(p, k0, C0_WIRED);
   i_addu(p, k1, k1, k0);
   i_mtc0(p, k1, C0_INDEX);
}
#endif

static __init void build_flush_entry(u32 **p, struct label **l,
                                        struct reloc **r,
                                        unsigned int k0,
					unsigned int k1, int idx)
{
    i_addiu(p, k0, 0, idx);
    i_sll(p, k0, k0, PAGE_SHIFT+1);
    i_addiu(p, k0, k0, CKSEG0);
    i_mfc0(p, k1, C0_ENTRYHI); /*save C0_ENTRYHI*/
    i_mtc0(p, k0, C0_ENTRYHI);
    i_addiu(p, k0, 0, idx);
    i_mtc0(p, k0, C0_INDEX);
    i_mtc0(p, 0, C0_ENTRYLO0);
    i_mtc0(p, 0, C0_ENTRYLO1);
    build_tlb_write_entry(p, l, r, tlb_indexed);
    i_mtc0(p, k1, C0_ENTRYHI); /*restore C0_ENTRYHI*/
}
#endif
...
#ifdef CONFIG_TIVO_WAR_FOR_BUG303720
    build_flush_entry(&p, &l, &r, K0, K1,0);
#endif
