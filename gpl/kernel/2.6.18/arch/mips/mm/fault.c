...
extern NORET_TYPE void my_die(const char *, struct pt_regs *);
#endif
/*
			      ...
			      unsigned long address)
{
	...
	siginfo_t info;
#ifdef  CONFIG_TIVO_PLOG
	plog_pagefault_ent(address, (unsigned long)regs->cp0_epc);
#endif
...
#endif
#ifdef CONFIG_TIVO
        _log_user_fault(__FUNCTION__, SIGBUS, regs, tsk);
#endif
	tsk->thread.cp0_badvaddr = address;
