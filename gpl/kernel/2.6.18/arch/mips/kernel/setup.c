...
#else
                /*
                 * Contigmem will carve out space
                 */
                size = ramSizeMB;
#endif
	}
...
static inline void parse_cmdline_early(void)
{
	...
	*to = '\0';
	...
	if (!usermem) {
                /* Let TiVo Contigmem subsytem allocate the reserved memory */
...
static inline int parse_rd_cmdline(unsigned long* rd_start, unsigned long* rd_end)
{
	...
	char *tmp;
#ifdef CONFIG_TIVO 
        struct tivo_rd_hdr *rd_hdr = (struct tivo_rd_hdr *)(PAGE_ALIGN((unsigned long)&_end));
        start = PAGE_ALIGN((unsigned long)&_end)+PAGE_SIZE;
        size = rd_hdr->size;
        if ( rd_hdr->magick == 0x64434431 ) 
            goto done;
        else {
            size = 0;
            start = 0;
