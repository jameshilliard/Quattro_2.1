...
#ifdef CONFIG_TIVO
			ohci_writel (ohci,
			((ohci_readl (ohci, &ohci->regs->roothub.a) & ~RH_A_PSM) | RH_A_NPS),
					&ohci->regs->roothub.a);
			ohci_writel (ohci, 0, &ohci->regs->roothub.b);
#endif /* CONFIG_TIVO */
			break;
		default:
			goto error;
...
#ifdef CONFIG_TIVO
			ohci_writel (ohci,
			((ohci_readl (ohci, &ohci->regs->roothub.a) & ~RH_A_NPS) | RH_A_PSM),
					&ohci->regs->roothub.a);
			ohci_writel (ohci, RH_B_PPCM, &ohci->regs->roothub.b);
#endif /* CONFIG_TIVO */
			break;
		default:
			goto error;
