...
static int usb_stor_scan_thread(void * __us)
{
	...
	unsigned int wait_for_sda;

	if (!has_sda && us->pusb_dev->bus->busnum != 2) {
		// suspend bus 1 if sda is not up
		printk(KERN_INFO "usb-storage: bus %d device pending on /dev/sda...\n",
			us->pusb_dev->bus->busnum);

		for (wait_for_sda = 0; !has_sda && wait_for_sda < 25; wait_for_sda++)
