...
/*******************************************************************************
Change Log:
[12/24/08]	Kevin Cernekee		e1000_main.c
		Define the EN2210+82545 driver name string.
		...
		In function int e1000_mii_ioctl()
...
#if defined(GMII_PROTO)

char e1000_driver_name[] = "e1000_z2";
#if defined(ZIP_2)
char e1000_driver_string[] = "i82545+EN2210 GMII Driver";
#else /* defined(ZIP_2) */
char e1000_driver_string[] = "i82545 GMII Driver";
#endif /* defined(ZIP_2) */

#else /* defined(GMII_PROTO) */

char e1000_driver_name[] = "e1000";
char e1000_driver_string[] = "Intel(R) PRO/1000 Network Driver";

#endif /* defined(GMII_PROTO) */
...
#undef DRV_VERSION
#define DRV_VERSION "8.0.1" DRV_NAPI DRV_DEBUG DRV_HW_PERF
...
#if defined(ZIP_2)
static int e1000_priv_ioctl(struct net_device *netdev, struct ifreq *ifr,
			   int cmd);
#endif /* defined(ZIP_2) */
...
#if defined(CONFIG_PM) && ! defined(GMII_PROTO)
...
static int e1000_request_irq(struct e1000_adapter *adapter)
{
	...
	err = request_irq(adapter->pdev->irq, (new_handler_t)&e1000_intr, irq_flags,
	                  netdev->name, netdev);
	if (err)
...
/***********************************************************
## Changes to facilitate initial programming of GMII boards.
## If checksum is 0xBABA (valid Intel board), 
## exit If checksum is 0x5A32 (valid GMII board), continue init
## If checksum is bad, print warning and continue 
## (so that user can run eeprg to fix it)     
**********************************************************/

	switch(e1000_validate_nvm_checksum(&adapter->hw))
        {
                case -E1000_ERR_NVM:
                        DPRINTK(PROBE, ERR, "EEPROM Checksum invalid - run eeprg to correct\n");
                        break;
                case -E1000_ERR_CONFIG:
                        DPRINTK(PROBE, INFO, "Found Intel board - skipping\n");
                        err = -EIO;
                        goto err_eeprom;
                default:
                        DEBUGOUT("Found GMII board with good EEPROM\n");
        }
    ...
    // init mailbox support for this device
#if 0001
    // create dg and dc contexts
    err = Clnk_init_dev( &adapter->p_dg_ctx, adapter, netdev, (unsigned long)adapter->hw.hw_addr ) ;
    if( err ) {
        DPRINTK(PROBE, ERR, " Clnk_init_dev err=%d.\n", err );
        goto err_register;
    }
#endif
...
#if 0001
    Clnk_exit_dev( adapter->p_dg_ctx );
    adapter->p_dg_ctx = 0 ;
#endif
...
{
    int          status = -EINVAL;
    dk_context_t *dkcp  = (dk_context_t *)netdev ;
...
#if defined(ZIP_2)
    case SIOCHDRCMD:
        return e1000_priv_ioctl(netdev, ifr, cmd);
#endif /* defined(ZIP_2) */
...
#if defined(ZIP_2)
static int
e1000_priv_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd)
{
    ...
    /* set reset SDP direction to "out" */
    ctrl_ext = E1000_READ_REG(hw, E1000_CTRL_EXT);
    ctrl_ext |= E1000_CTRL_EXT_ZIP_RESET_DIR | E1000_CTRL_EXT_BITS;

    /* set or clear EN2210 reset pin, based on user-supplied flag */

	/* NOTE: PLL and SoC core should be reset individually. SoC core should reset
	 * after PLL by 50ms according to data sheet. While in our GMII PC card hardware
	 * design, SoC core and PLL reset pins are tied together, and share the same
	 * address E1000_CTRL_EXT_ZIP_RESET. It's really a bad design. We use the address
	 * E1000_CTRL_EXT_ZIP_RESET for both SoC core and PLL reset at present to emphasis
	 * that the reference design should follow the steps to reset PLL, and then reset
	 * SoC core, and make sure the time interval should not less than 50ms. --Jeffrey*/

    if(flag == 1) // hold SoC
        ctrl_ext |= E1000_CTRL_EXT_ZIP_RESET;
    else if(flag == 0) // run SoC
        ctrl_ext &= ~E1000_CTRL_EXT_ZIP_RESET;
	else if(flag == 4) // hold PLL
        ctrl_ext |= E1000_CTRL_EXT_ZIP_RESET;
	else // flag == 5, run PLL
        ctrl_ext &= ~E1000_CTRL_EXT_ZIP_RESET;
