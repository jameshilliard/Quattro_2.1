...
static int e1000_intr_test(struct e1000_adapter *adapter, uint64_t *data)
{
	...
	*data = 0;
	...
	if (!request_irq(irq, (new_handler_t)&e1000_test_intr, IRQF_PROBE_SHARED,
	                 netdev->name, netdev))
		shared_int = FALSE;
	else if (request_irq(irq, (new_handler_t)&e1000_test_intr, IRQF_SHARED,
	                     netdev->name, netdev)) {
		*data = 1;
		return -1;
	}
