...
#define CONFIG_THOMSON_EXTPHY

#if defined(CONFIG_THOMSON_EXTPHY)
/* Temporary until we get an official 7335B0 kernel */
/* SUN_TOP_CTRL :: PIN_MUX_CTRL_11 :: gpio_073 [02:00] */
#define BCHP_SUN_TOP_CTRL_PIN_MUX_CTRL_11_gpio_073_MASK_B0            0x00000007
#define BCHP_SUN_TOP_CTRL_PIN_MUX_CTRL_11_gpio_073_SHIFT_B0           0
/* SUN_TOP_CTRL :: PIN_MUX_CTRL_11 :: gpio_074 [05:03] */
#define BCHP_SUN_TOP_CTRL_PIN_MUX_CTRL_11_gpio_074_MASK_B0            0x00000038
#define BCHP_SUN_TOP_CTRL_PIN_MUX_CTRL_11_gpio_074_SHIFT_B0           3

/* used to select topology of GPIO output buffer*/
#define GIO_ODEN_EXT            (BCHP_GIO_REG_START + 0x40)
#define GIO_DATA_EXT            (BCHP_GIO_REG_START + 0x44)
#define GIO_IODIR_EXT           (BCHP_GIO_REG_START + 0x48)
/* The bits 8 - 31 reflects the state of pins 64-87 */
#define GIO_GPIO73_EXT        (0x00080000)
#define GIO_GPIO74_EXT        (0x00100000)

/* local masks used to decide the PHY used with the second MAC */ 
#define TM_ENABLE_MOCA_PHY	0x01
#define TM_ENABLE_ETH_PHY	0x02
#endif /* CONFIG_THOMSON_EXTPHY */

/* Flow control/Pause frame registers */
#define EMAC1_IUDMA_CNTRL_CFG 	*((volatile unsigned long *)(KSEG1ADDR(0x10000000 + 0x0092400)))
#define EMAC1_RX_CONFIG 		*((volatile unsigned long *)(KSEG1ADDR(0x10000000 + 0x0090000)))
#define EMAC1_Xmit_Control		*((volatile unsigned long *)(KSEG1ADDR(0x10000000 + 0x0090030)))
#define EMAC1_FCLowThr_MASK 	0x2
#define EMAC1_ENFLOW_MASK		0x20
#define EMAC1_FD_MASK			0x1
...
#if (defined(CONFIG_THOMSON_EXTPHY) && defined(CONFIG_TIVO_KONTIKI))
/* Changes the output from GPIO pins to activate the desired external PHY */
int thomson_set_phy_enable(unsigned char mask);
#endif
...
#ifdef CONFIG_TIVO_KONTIKI
static uint8 g_flash_eaddrs[2][ETH_ALEN];
#else
static uint8 g_flash_eaddr[ETH_ALEN];
#endif

#if (defined(CONFIG_THOMSON_EXTPHY) && defined(CONFIG_TIVO_KONTIKI))
static int Eth1PhySel = 0;
module_param(Eth1PhySel, int, S_IRUGO);
/* MakeMocaEth0 shall be set to 1 in the case where kernel wants to make Moca PHY eth0 */
static short MakeMocaEth0 = 0;
module_param(MakeMocaEth0, short, S_IRUGO);
#endif
...
#if ((defined(CONFIG_TIVO) && defined(CONFIG_BCMINTEMAC_NETLINK)) \
     || defined(CONFIG_TIVO_KONTIKI))
...
#if (defined(DISABLE_INTERRUPTS) && defined(CONFIG_TIVO))
        ...
        if (linkState != pDevCtrl->linkState) {
            if (linkState != 0) {
                if (pDevCtrl->EnetInfo.ucPhyType == BP_ENET_EXTERNAL_SWITCH) {
                    ...
                    {
                       /*
                         In case of HR24, the external PHY is the c.LINK chip - EN2220.
                         The chip advertises full duplex support by default, even though the underlying MoCA network is not.
                         The EN2220 sends PAUSE frames to the MAC - which is characteristic of a PHY that is linking to a full duplex network.
                       */
                       v = MII_AUX_CTRL_STATUS_FULL_DUPLEX;
                    }
                    else
                    {
                    ...
                    if( (v & MII_AUX_CTRL_STATUS_FULL_DUPLEX) != 0) {
                        ...
                        pDevCtrl->emac->txControl &= ~EMAC_FD;
#else /* #if (defined(CONFIG_THOMSON_EXTPHY) && defined(CONFIG_TIVO_KONTIKI)) */           
                    ...
                    if( (v & MII_AUX_CTRL_STATUS_FULL_DUPLEX) != 0) {
                        ...
                        pDevCtrl->emac->txControl &= ~EMAC_FD;
#endif /* #if (defined(CONFIG_THOMSON_EXTPHY) && defined(CONFIG_TIVO_KONTIKI)) */
                }
...
#if (defined(CONFIG_TIVO) && !defined(CONFIG_TIVO_KONTIKI))
                // mkm - the following is to get around some overzealous
                // bounds checking that notices that pBuf-4 could become
                // negative and causes TiVo's compiler to complain about
                // a load/store address overflow
                if (bufferAddress >= 4) {
...
/*----------------------------- SKBs --------------------------------*/
    ...
    case PROC_DUMP_EMAC_REGISTERS: /* EMAC registers */
        ...
        break;
#ifdef CONFIG_TIVO_KONTIKI
	case PROC_DUMP_CLINK: /* c.LINK link status */
        len += sprintf(&buf[len], "\nc.LINK link status:\n");
        val_out = mii_read(pDevCtrl->dev, pDevCtrl->EnetInfo.ucPhyAddress & 0x1f, 0x1);
		if (val_out & 0x4)
        	len += sprintf(&buf[len], "c.LINK is UP\n");
		else
        	len += sprintf(&buf[len], "c.LINK is DOWN\n");
		break;
#endif /* CONFIG_TIVO_KONTIKI */
...
#ifdef CONFIG_TIVO_KONTIKI
    if (pos >= PROC_DUMP_DMADESC_STATUS && pos <= PROC_DUMP_CLINK) {
...
#ifdef CONFIG_TIVO_KONTIKI
		for (i=0; i < 6; i++)
			g_flash_eaddrs[0][i] = (uint8) gHwAddrs[0][i];
		if(gNumHwAddrs >= 2) {
			for (i=0; i < 6; i++)
				g_flash_eaddrs[1][i] = (uint8) gHwAddrs[1][i];
		}
#else
		for (i=0; i < 6; i++)
...
#ifdef CONFIG_TIVO_NEUTRON
// support for /proc/ar8328, return current link state 0 to 3 
static int ar8328_read_proc(char *buf, char **start, off_t offset, int len, int *eof, void *data)
{
    int n=0;
    if (!offset) n = sprintf(buf, "%d\n", ((BcmEnet_devctrl *)data)->linkState);
    buf[n]=0; 
    *eof=1;
    return n;
}  
...
#ifdef CONFIG_TIVO_KONTIKI
    if (0 <= devnum && devnum <= 1)
    {
       memcpy(dev->dev_addr, g_flash_eaddrs[devnum], ETH_ALEN);
#if 1 /* ###JLF */
       printk("%s(): dev[%d]: MAC address "
              "%02x:%02x:%02x:%02x:%02x:%02x\n",
              __FUNCTION__, devnum,
              g_flash_eaddrs[devnum][0], g_flash_eaddrs[devnum][1], g_flash_eaddrs[devnum][2],
              g_flash_eaddrs[devnum][3], g_flash_eaddrs[devnum][4], g_flash_eaddrs[devnum][5]);
#endif
    }
#else
...
#ifdef CONFIG_TIVO_NEUTRON
    if (boardID & kTivoConfigGen07Strap_IsBridge)  
    {
        // configure bridge and build /proc/ar8328 
        ar8328_config(dev);
        create_proc_read_entry("ar8328", 0, NULL, ar8328_read_proc, pDevCtrl); 
    }
#endif    
...
#if (defined(CONFIG_THOMSON_EXTPHY) && defined(CONFIG_TIVO_KONTIKI))
/*
Sets function callbacks that linux can call into
*/
static void bcmemac_set_mii_defaults(BcmEnet_devctrl *pDevCtrl)
{
    pDevCtrl->mii.phy_id_mask = 0x1F;
    pDevCtrl->mii.reg_num_mask = 0x1F;
    pDevCtrl->mii.dev = pDevCtrl->dev;
...
#if (defined(CONFIG_THOMSON_EXTPHY) && defined(CONFIG_TIVO_KONTIKI))
		/* 
			If Moca PHY should be eth0, then make it so. Then Ethernet will have to be eth1.
			We do not attempt to name the interface for denum > 1.
		 */
		if (MakeMocaEth0 == 1 && devnum <= 1)
        	dev = alloc_netdev(sizeof(struct BcmEnet_devctrl), devnum ? "eth0" : "eth1", bcmemac_null_setup);
		else
#endif
...
#if (defined(CONFIG_THOMSON_EXTPHY) && defined(CONFIG_TIVO_KONTIKI))
    /* 
    This is being called for both eth0 and eth1. It is only relevant in case of eth1 as only MAC_1
    is connected to PHY over MII. This supports the implementation of ETHTOOL IOCTLs 
    */
    bcmemac_set_mii_defaults(pDevCtrl); 
#endif
...
#ifdef CONFIG_TIVO_NEUTRON
static int force_bcmemac1_up=0;
EXPORT_SYMBOL(force_bcmemac1_up);
#endif
...
#if (!defined(CONFIG_TIVO_KONTIKI) && !defined(CONFIG_TIVO_NEUTRON))
...
#ifdef CONFIG_TIVO_NEUTRON
            // allow helium emoca driver to force emac1 link status up 
            // note we OR in a 2, this will be reported via /proc/ar8328
            if (pDevCtrl->devnum == 1) linkState |= (force_bcmemac1_up?2:0);
#endif            
            pDevCtrl->linkstatus_polltimer = 0;
        }
...
#endif
#ifdef CONFIG_TIVO_NEUTRON
        remove_proc_entry("ar8328", NULL); // OK if this doesn't exist
#endif    
    ...
    case ETHTOOL_GSET:
        {
            ...
            /* 
            Using the below standard linux function for doing ethtool get helps in packaging
            the returned value into the ethtool_cmd structure.
            Also, note that the below call ends up calling the mii_read function implemented in this
            ethernet driver. 
            */	
            mii_ethtool_gset(&pDevCtrl->mii, &et_settings); 
            copy_to_user(useraddr, &et_settings, sizeof(et_settings)); 
            return 0;
        }
    case ETHTOOL_SSET:
        {
            struct ethtool_cmd et_settings;
            copy_from_user(&et_settings, useraddr, sizeof(et_settings));
            /*
            Using the below standard linux function for doing ethtool set helps in converting
            the values in ethtool_cmd structure to the binary pattern.
            Also, note that the below call ends up calling the mii_write function implemented in this
            ethernet driver.	
            */
            mii_ethtool_sset(&pDevCtrl->mii, &et_settings); 
			return 0;
         }
    case ETHTOOL_GPAUSEPARAM:
		{
			...
			/*
			Auto-negotiation of pause params not supported.
			Only if the RX_CONFIG.ENFLOW and TX_CONFIG.FD bits are set, the RX of PAUSE frames is enabled.
			If the FCLowThr bit in the IUDMA_CNTRL_CFG register is set then it means the TX of PAUSE frames is enabled. 
			*/
			value = EMAC1_IUDMA_CNTRL_CFG; 	
			epause.tx_pause = value & EMAC1_FCLowThr_MASK; 	 
			value = EMAC1_Xmit_Control;
			if (value & EMAC1_FD_MASK)
			{
				value = EMAC1_RX_CONFIG;
				epause.rx_pause = value & EMAC1_ENFLOW_MASK;
			}
			copy_to_user(useraddr, &epause, sizeof(epause)); 
			return 0; 
		}
    case ETHTOOL_SPAUSEPARAM:
		{
			struct ethtool_pauseparam epause;
			unsigned long value;
			copy_from_user(&epause, useraddr, sizeof(epause));
			/* The MAC does not support auto negotiation of pause params */
			if (epause.autoneg != AUTONEG_ENABLE)
			{
				/* Set the FCLowThr bit in the IUDMA_CNTRL_CFG register to enable TX of PAUSE frames */
				value = EMAC1_IUDMA_CNTRL_CFG; 	
				if (epause.tx_pause)
					EMAC1_IUDMA_CNTRL_CFG = value | EMAC1_FCLowThr_MASK;
				else
					EMAC1_IUDMA_CNTRL_CFG = value & ~EMAC1_FCLowThr_MASK;

				/* Set RX_CONFIG.ENFLOW and TX_CONFIG.FD bits to enable processing of RX PAUSE frames */
				value = EMAC1_Xmit_Control;
				if (epause.rx_pause)
				{
					EMAC1_Xmit_Control = value | EMAC1_FD_MASK;
					value = EMAC1_Xmit_Control;
					value = EMAC1_RX_CONFIG;
					EMAC1_RX_CONFIG = value | EMAC1_ENFLOW_MASK; 
				}
				else
				{
					value = EMAC1_RX_CONFIG;
...
static int bcmemac_enet_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
    ...
    case SIOCETHTOOL:
        val = netdev_ethtool_ioctl(dev, (void *) rq->ifr_data);
    ...
    case SIOCGIFHWADDR:
        {
            ...
            for(i = 0; i < g_num_devs; i++) {
                if (!strcmp(g_devs[i]->name, rq->ifr_name)) 
                {
                    memcpy(rq->ifr_hwaddr.sa_data, g_devs[i]->dev_addr, 6);
                    val = 0;
                    break;
                }
            }
    ...
    case SIOCDEVPRIVATE:
        // do a true hardware reset of the EN2220. Pulling down RESET_TO_CORE (U16) pin on Entropic for min. 10us will reset
        // all components except PCI core. Thomson has PCI reset pulled to GND. Also PCI core need not be reset as the PCI mode is not used. 
        // The GPIO 72 (pin AJ29) is the MOCA reset line.
        thomson_set_phy_enable(0); // disable all the PHYs
        udelay(100);
        thomson_set_phy_enable(TM_ENABLE_MOCA_PHY);
        val = 0;
        break;
#endif /* CONFIG_THOMSON_EXTPHY */
...
#if (defined(CONFIG_THOMSON_EXTPHY) && defined(CONFIG_TIVO_KONTIKI))
/*
Sets the appropriate pin mux settings to enable c.LINK or the Ethernet.
mask: bit 0 - used for moca setting. bit 1 - used for ethernet setting.
As both the GPIOs (for Moca and Ethernet) cannot be ON at the same time, a value of 1 for both the bits is not permitted.
*/
int thomson_set_phy_enable(unsigned char mask)
{
    unsigned long value;
    ...
    enable_moca = mask & TM_ENABLE_MOCA_PHY;
    enable_ethernet = mask & TM_ENABLE_ETH_PHY;
    // cannot enable both of them at the same time
    if (enable_moca && enable_ethernet)
        return -1;

    /* Override the MUX Value */
    value = BDEV_RD(BCHP_SUN_TOP_CTRL_PIN_MUX_CTRL_11);
    BDEV_WR(BCHP_SUN_TOP_CTRL_PIN_MUX_CTRL_11, value & ~(BCHP_SUN_TOP_CTRL_PIN_MUX_CTRL_11_gpio_074_MASK_B0 | BCHP_SUN_TOP_CTRL_PIN_MUX_CTRL_11_gpio_073_MASK_B0));

    /* Override the IODIR - Make sure output */
    value = BDEV_RD(GIO_IODIR_EXT);
    BDEV_WR(GIO_IODIR_EXT, value & ~(GIO_GPIO74_EXT | GIO_GPIO73_EXT));

    /* Override the ODEN - Make sure totem pole */
    value = BDEV_RD(GIO_ODEN_EXT);
    BDEV_WR(GIO_ODEN_EXT, value & ~(GIO_GPIO74_EXT | GIO_GPIO73_EXT));

    value = BDEV_RD(GIO_DATA_EXT);
    if (enable_moca)
    {
        value |= GIO_GPIO73_EXT;
        value &= ~GIO_GPIO74_EXT;
    }
    else
    {
        if (enable_ethernet)
        ...
        {
            // if we get here, both moca and ethernet are to be disabled.
            value &= ~GIO_GPIO73_EXT;
            value &= ~GIO_GPIO74_EXT;
        }
    }
    BDEV_WR(GIO_DATA_EXT, value);
...
#if (defined(CONFIG_THOMSON_EXTPHY) && defined(CONFIG_TIVO_KONTIKI))
    thomson_set_phy_enable(Eth1PhySel & 3);
#endif
...
#ifdef CONFIG_TIVO_NEUTRON
    if (!(boardID & kTivoConfigGen07Strap_IsBridge)) // avoid EMAC0 
#endif        
    if(! status)
...
#ifdef CONFIG_TIVO_NEUTRON
        if (boardID & kTivoConfigGen07Strap_IsBridge) phy_id=AR8328_ETH_PHY; 
        else
#endif        
...
/* The following functions are called by the emoca driver. In some cases wrappers
 * were defined to ensure the function names are unique within the kernel namespace
 */
EXPORT_SYMBOL(bcmemac_net_open);
EXPORT_SYMBOL(bcmemac_net_close);
EXPORT_SYMBOL(bcmemac_net_xmit);
EXPORT_SYMBOL(bcmemac_net_timeout);
EXPORT_SYMBOL(bcmemac_net_query);
EXPORT_SYMBOL(bcmemac_net_isr);
EXPORT_SYMBOL(bcmemac_enet_poll);
EXPORT_SYMBOL(bcmemac_link_change_task);
EXPORT_SYMBOL(bcmemac_init_dev);
EXPORT_SYMBOL(bcmemac_uninit_dev);

int bcmemac_set_mac_addr(struct net_device *dev, void *p)
{
	return(bcm_set_mac_addr(dev, p));
}
EXPORT_SYMBOL(bcmemac_set_mac_addr);

void bcmemac_set_multicast_list(struct net_device * dev)
{
	bcm_set_multicast_list(dev);
}
EXPORT_SYMBOL(bcmemac_set_multicast_list);

void bcmemac_tx_reclaim_timer(unsigned long arg)
{
	tx_reclaim_timer(arg);
}
EXPORT_SYMBOL(bcmemac_tx_reclaim_timer);

void bcmemac_write_mac_address(struct net_device *dev)
{
	write_mac_address(dev);
}
EXPORT_SYMBOL(bcmemac_write_mac_address);

bool bcmemac_bridge_enabled(void)
{
        if (boardID & kTivoConfigGen07Strap_IsBridge)
            return TRUE;
        else
...
EXPORT_SYMBOL(bcmemac_bridge_enabled);
