...
// DVT testmode
// 0  : normal
// 1  : 100BaseT force mode
// 2  : 100BaseT force loopback mode
...
__setup("bcmiitestmode=", do_testmode);
#endif 
...
static void mii_force(struct net_device *dev);
#endif
...
int mii_init(struct net_device *dev)
{
    ...
    printk(KERN_INFO "Config %s Through %s\n", phytype, setup);
    ...
    switch(pDevCtrl->EnetInfo.ucPhyType) {
        ...
        case BP_ENET_EXTERNAL_PHY:
            ...
            /* reset phy */
#ifdef CONFIG_TIVO_KONTIKI
            /* In HR24, the second PHY (EN2220) does not support the standard MII control register entry for soft reset.
               So the soft reset call is commented out below.
               TODO: We need to check phy_id to make sure the PHY is actually EN2220 before taking out the soft reset */
#else
            mii_soft_reset(dev, pDevCtrl->EnetInfo.ucPhyAddress);
#endif
...
/* The following functions are called by the emoca driver. The wrappers were
 * defined to ensure the function names are unique within the kernel namespace
 */
int bcmemac_mii_read(struct net_device *dev, int phy_id, int location)
{
	return(mii_read(dev, phy_id, location));
}
EXPORT_SYMBOL(bcmemac_mii_read);

void bcmemac_mii_write(struct net_device *dev, int phy_id, int location, int val)
{
	mii_write(dev, phy_id, location, val);
}
EXPORT_SYMBOL(bcmemac_mii_write);
