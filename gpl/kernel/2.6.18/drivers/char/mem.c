...
// Need plog on release temporarily...
#define CONTIGMEM_BARRIER_SIZE                  PAGE_SIZE
#define DEFAULT_CONTIGMEM_PLOG_SIZE             (128 * 1024)
#endif
...
#define RESERVE_TIVO_STICKY_PAGE   1

#ifdef RESERVE_TIVO_STICKY_PAGE
#define WARM_REBOOT_MAGIC_VALUE   0xFACEDEED
#endif
...
void __init contigmem(void) {
        ...
        if (physmem_top > 256 * 1024 * 1024) {
                contigmem_top = (unsigned long)__va(256 * 1024 * 1024);
        } else {
                contigmem_top = (unsigned long)__va(physmem_top);
        }

#ifdef RESERVE_TIVO_STICKY_PAGE
        /* subtract off 1 page for sticky region */
        contigmem_top -= PAGE_SIZE;
#endif

        printk("contigmem_top = 0x%lx\n", contigmem_top);
        ...
        if (contigmem_start < (unsigned long)&_end + CONTIGMEM_MINLEFT) {
                contigmem_start = PAGE_ALIGN((unsigned long)&_end + CONTIGMEM_MINLEFT);
                if (contigmem_start + contigmem_total > (unsigned long)__va(physmem_top)) {
                        contigmem_start = 0;
                        for (i = 0; i <= MAX_CONTIGMEM_REGION; i++) {
                                contigmem_size[i] = 0;
                        }
        ...
        reserve_bootmem(__pa(contigmem_start), contigmem_total);
...
#ifdef RESERVE_TIVO_STICKY_PAGE
        /* reserve the sticky page */
        reserve_bootmem(__pa(contigmem_top), PAGE_SIZE);

        /* check previous reboot status, last 4 bytes of reserved sticky page */
        unsigned long *pRebootStatus = (unsigned long *)(contigmem_top + PAGE_SIZE - 4);
        
        if (*pRebootStatus == WARM_REBOOT_MAGIC_VALUE) {
            printk("Detected warm reboot.\n");
        } 
        else {
	    printk("Detected likely cold reboot (power pulled). Other reasons could include booting from an older kernel. Value=0x%lx\n", *pRebootStatus);
        }
        *pRebootStatus = WARM_REBOOT_MAGIC_VALUE;
#endif
}
