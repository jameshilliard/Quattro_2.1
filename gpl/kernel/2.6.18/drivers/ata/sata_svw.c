...
int k2_sata_scontrol_read(struct ata_port* ap)
{
        void __iomem *port = (void __iomem *)ap->ioaddr.cmd_addr;

        return (readl(port + K2_SATA_SCR_CONTROL_OFFSET) & 0x000f0000) >> 16;
}
#endif /* !defined(CONFIG_TIVO_KONTIKI) */
...
#if (defined(CONFIG_TIVO_KONTIKI) && defined(CONFIG_MIPS_BCM_NDVD))
...
// The affects the operation of sata_pmp_scr_read and sata_pmp_scr_write, below.
// Accesses to the suppressed port do nothing, successfully.  
int suppress_pmp = -1; // by default, do not suppress
#endif
...
static void k2_sata_pmp_attach(struct ata_port *ap)
{
	...
	{
		u32 ss;
		if (suppress_pmp < 0 &&						// if not already suppressed
		    ap->nr_pmp_links &&						// and we have pmp port 1
		    !sata_pmp_scr_read(&ap->pmp_link[1], SCR_STATUS, &ss) &&	// and we can read SStatus register   
		    (ss & 1))							// and a device is detected	  
		{
			// then e-sata is connected, suppress port 0
			printk("E-SATA detected, disabling internal drive.\n");
			suppress_pmp = 0;  
		}
		else 
		{
			// else e-sata is not connected, make sure it doesn't show up later
			suppress_pmp = 1;
		}    
	}
#endif    
...
static int k2_sata_do_softreset(struct ata_link *link, unsigned int *class, int pmp)
{
	...
	struct ata_taskfile tf;
#ifdef CONFIG_TIVO_KONTIKI
  /* X.Ji Added retry to the signature reading below. The reading failed on some HDD because the HDD is 
     too slow and still powering up when the reading happens. The failure caused device type not and
     eventually leads to IRD reboot. The exact retry time could be fine tuned once we know how long
     it takes for the HDD to power up. */
  unsigned int tclass;
  int nretry;
  const static int maxretry = 10; 
#endif /* CONFIG_TIVO_KONTIKI */
  ...
  do
  {
	...
	ap->ops->tf_read(ap, &tf);
#ifdef CONFIG_TIVO_KONTIKI
		tclass = ata_dev_classify(&tf);

		if (tclass == ATA_DEV_UNKNOWN)
		{
			msleep(1000);
		}
		else
...
static int k2_sata_init_one (struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	probe_ent->irq_flags = IRQF_SHARED|IRQF_SAMPLE_RANDOM;
#else
...
#ifdef CONFIG_TIVO_KONTIKI
	/*
	 * TKR: On the HR24 we need to swap controler 0 and 1 so that the controller
	 * connected to the interenal drive will be instanciated before the external.
	 * Basically we're correcting a hardware decision.
	 */
#endif
	for (i = 0; i < ent->driver_data; i++)
        {
#ifdef CONFIG_TIVO_KONTIKI
		k2_sata_setup_port(&probe_ent->port[i], base + (i ^ 1) * K2_SATA_PORT_OFFSET);
#else
