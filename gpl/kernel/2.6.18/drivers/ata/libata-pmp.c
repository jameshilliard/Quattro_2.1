...
/*
 * Program signal level controls for Marvel 4140 PM on a per-port basis.
 */
static int sata_pmp_marvell_levels(struct ata_device *dev, u32 port,
								  u32 txamp, u32 txpre, u32 pre_half_en, u32 ibiasx2_en, u32 drv_en)
{
    ...
    /*
     * Make sure port is offline.  Will be put back on line if needed
     * by the normal port initialization step
     */
	rc = ap->ops->pmp_write(dev, port, SATA_PMP_PSCR_CONTROL, 0x004);
    if(rc)
        return rc;  /* don't know how to do error handling */

    /*
     * Limit the speed to 1.5Gbs.
     */
    rc = ap->ops->pmp_write(dev, port, SATA_PMP_PSCR_CONTROL, 0x014);
    if(rc)
        return rc;  /* don't know how to do error handling */

    /* Use undocumented register to select the  port to progam */
	rc = ap->ops->pmp_write(dev, link->pmp, 0x8C, port);
    if(rc)
        return rc;  /* don't know how to do error handling */

    /* get contents of Marvell specific Phy Mode 2 register */
	rc = ap->ops->pmp_read(dev, link->pmp, 0x91, &reg);
    if(rc)
        return rc;  /* don't know how to do error handling */

    /* update txamp and txpre fields leaving other bits unchanged */
	reg &= ~((0xf << 7) | (0x7 << 4));
	reg |=  (txamp << 7) | (txpre << 4);
	rc = ap->ops->pmp_write(dev, link->pmp, 0x91, reg);
    if(rc)
        return rc;  /* don't know how to do error handling */

    /* get contents of Marvell specific Phy Mode 7 register */
	rc = ap->ops->pmp_read(dev, link->pmp, 0x9d, &reg);
    if(rc)
        return rc;  /* don't know how to do error handling */

    /* update pre_half_en, ibiasx2_en and drv_en fields leaving other bits unchanged */
	reg &= ~((1 << 9) | (1 << 8) | 0x3f);
	reg |=  (pre_half_en << 9) | (ibiasx2_en << 8) | drv_en;
	rc = ap->ops->pmp_write(dev, link->pmp, 0x9d, reg);
    if(rc)
        return rc;  /* don't know how to do error handling */
...
static void sata_pmp_quirks(struct ata_device *dev, int *nr_ports)
{
	...
	/* configure IO levels for Marvel PM */
	if(vendor == 0x11AB && devid == 0x4140)
	{
		sata_pmp_marvell_levels(dev, 0, 1, 0, 1, 0, 0x2a);
		sata_pmp_marvell_levels(dev, 1, 3, 0, 1, 0, 0x2a);
	}
}
