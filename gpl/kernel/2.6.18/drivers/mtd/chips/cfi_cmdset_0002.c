...
// convert slave mtd to master mtd
extern struct mtd_info * get_master(struct mtd_info * mtd);

// lock or verify S29 boot block, wrongly called directly from mtdchar.c 
int mojave_bbl(struct mtd_info *slave_mtd, int verify)
{
	struct mtd_info *master_mtd = get_master(slave_mtd);
	struct map_info *map = master_mtd->priv;
	struct cfi_private * cfi = map->fldrv_priv;  
	struct flchip *chip = cfi->chips;   // just one
	...
	if (!verify)
	{
		printk(KERN_NOTICE "Locking NOR boot block\n");
    
		if ((ret = get_chip(map, chip, 0, FL_WRITING)) != 0) goto fail;

		ENABLE_VPP(map);

		// per S29GL datasheet section 9.13, the global PPB lock must be done first
		cfi_send_gen_cmd(0xAA, cfi->addr_unlock1, chip->start, map, cfi, cfi->device_type, NULL);
		cfi_send_gen_cmd(0x55, cfi->addr_unlock2, chip->start, map, cfi, cfi->device_type, NULL);
		cfi_send_gen_cmd(0x40, cfi->addr_unlock1, chip->start, map, cfi, cfi->device_type, NULL);
		map_write(map,(map_word){{0xa0}},0);
		map_write(map,(map_word){{0xfffd}},0); //  DQ2=1, DQ1=0, DQ0=1

		timeout = jiffies + HZ;
		while (!chip_ready(map, 0)) if (time_after(jiffies, timeout))
		{
			printk(KERN_ERR "Timeout programming lock register\n");
			ret = -EIO;
			break;
		}
		...
		if (!ret)
		{
			...
			// then set PPB for first two sectors
			cfi_send_gen_cmd(0xAA, cfi->addr_unlock1, chip->start, map, cfi, cfi->device_type, NULL);
			cfi_send_gen_cmd(0x55, cfi->addr_unlock2, chip->start, map, cfi, cfi->device_type, NULL);
			cfi_send_gen_cmd(0xc0, cfi->addr_unlock1, chip->start, map, cfi, cfi->device_type, NULL);

			unsigned int x;
			for (x = 0; !ret && x < 2; x++)                         // two secrors
			{
				map_write(map,(map_word){{0xa0}},0);
				map_write(map,(map_word){{0x00}},x*0x20000);        // 128k bytes per sector
				timeout = jiffies + HZ;
				while (!chip_ready(map, x))
				if (time_after(jiffies, timeout))
				{
					printk(KERN_ERR "Timeout programming PPB%d\n", x);
					ret = -EIO;
					break;
				}
			}
	...
	{
		printk(KERN_NOTICE "Verifying NOR boot block\n");

		if ((ret = get_chip(map, chip, 0, FL_READY)) != 0) goto fail;

		cfi_send_gen_cmd(0xAA, cfi->addr_unlock1, chip->start, map, cfi, cfi->device_type, NULL);
		cfi_send_gen_cmd(0x55, cfi->addr_unlock2, chip->start, map, cfi, cfi->device_type, NULL);
		cfi_send_gen_cmd(0x40, cfi->addr_unlock1, chip->start, map, cfi, cfi->device_type, NULL);

		map_word lock = map_read(map, 0);

		cfi_send_gen_cmd(0x90, cfi->addr_unlock1, chip->start, map, cfi, cfi->device_type, NULL);
		cfi_send_gen_cmd(0x00, cfi->addr_unlock1, chip->start, map, cfi, cfi->device_type, NULL);

		// and the PPB registers
		cfi_send_gen_cmd(0xAA, cfi->addr_unlock1, chip->start, map, cfi, cfi->device_type, NULL);
		cfi_send_gen_cmd(0x55, cfi->addr_unlock2, chip->start, map, cfi, cfi->device_type, NULL);
		cfi_send_gen_cmd(0xc0, cfi->addr_unlock1, chip->start, map, cfi, cfi->device_type, NULL);
		map_word PPB0 = map_read(map, 0);          // sector 0
		map_word PPB1 = map_read(map, 0x20000);    // sector 1

		printk("Got LOCK=%lX PPB0=%lX PPB1=%lX\n", lock.x[0], PPB0.x[0], PPB1.x[0]);
		if (lock.x[0] & 2 || PPB0.x[0] & 1 || PPB1.x[0] & 1) ret=-EINVAL;
	}    

	// we're done, return to array mode and release the chip 
	cfi_send_gen_cmd(0x90, cfi->addr_unlock1, chip->start, map, cfi, cfi->device_type, NULL);
	cfi_send_gen_cmd(0x00, cfi->addr_unlock1, chip->start, map, cfi, cfi->device_type, NULL);
