		...
		unsigned int sector, monotonic_t *deadline)
{
    ...
    ret = blk_rq_map_user_iov(q, rq, hdr->dxferp, hdr->iovec_count);
    ...
    rq->hard_nr_sectors = rq->nr_sectors = rq->current_nr_sectors = (hdr->dxfer_len >> 9);
...
#ifdef CONFIG_TIVO
    plog_diskdispatch(sector, rq->nr_sectors);
#endif
            ...
            if((byte_count + cb) > MAX_BYTES){
                ...
                else if(byte_count == 0 && cb > MAX_BYTES)
                {
                    ...
                    while(length > 0){
                        ...
                        if(iov_table.iov_len % SECTOR_BYTES){
                            printk("scsi_dosector: iovec bytes(%u) not aligned on sector boundary\n",
                                    iov_table.iov_len);
                            BUG();
                        }
