...
struct scsi_device *scsi_dev_get(struct scsi_disk *sdkp)
{
        struct scsi_device *sdev = sdkp->device;
	return sdev;
}
...
static int sd_init_command(struct scsi_cmnd * SCpnt)
{
	 ...
	 * we typically use 1K nr_sectorsizes, and cdroms typically have
...
#if defined (CONFIG_SCSI_DOSECTORS) && defined (CONFIG_TIVO)
        /*Nasty. But no option. Providing backward compatibility for 
          some legacy IDE stuff*/
         if ( cmd == 0x31b )
                cmd = SCSI_IOCTL_DOSECTORS;
#endif
	/*
...
#ifdef CONFIG_SCSI_DOSECTORS
extern int scsi_dosectors(struct block_device *bdev, void __user *arg);

		case SCSI_IOCTL_DOSECTORS:
			return scsi_dosectors(bdev, p);
#endif
...
int has_sda=0;	// see usb-storage, do_mounts
#endif
...
static int sd_probe(struct device *dev)
{
	...
	if (!index)
	{
		printk(KERN_INFO "/dev/sda is up\n");
		has_sda=1;
	}
#endif
