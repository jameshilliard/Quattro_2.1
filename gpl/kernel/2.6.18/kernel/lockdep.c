			  ...
			  unsigned long ip)
{
	...
	MARK(kernel_lock_acquire, "%lu %u struct lockdep_map %p %d",
...
__lock_release(struct lockdep_map *lock, int nested, unsigned long ip)
{
	...
	MARK(kernel_lock_release, "%lu struct lockdep_map %p %d",
		ip, lock, nested);
