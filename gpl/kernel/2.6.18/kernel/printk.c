...
/************* END HACK END HACK END HACK **********************************/
	...
	if (printed_len > 0) {
		...
		if (mark_buf[mark_len-1] == '\n')
			mark_len--;
		MARK(kernel_vprintk, "%hu %d %s %p",
			loglevel, mark_len, mark_buf,
...
void get_syslog_data(char *syslog_data[4]) {
        syslog_data[0] = __log_buf;
        syslog_data[1] = __log_buf + sizeof(__log_buf);
        syslog_data[2] = __log_buf + log_end - (logged_chars < __LOG_BUF_LEN ? logged_chars : __LOG_BUF_LEN);
        syslog_data[3] = __log_buf + log_end;
}
#endif
