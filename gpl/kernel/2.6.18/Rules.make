...
$(MODINCL)/%.ver: %.c
	@if [ ! -r $(MODINCL)/$*.stamp -o $(MODINCL)/$*.stamp -ot $< ]; then \
		...
		$(CC) $(CFLAGS) $(EXTRA_CFLAGS_nostdinc) -E -D__GENKSYMS__ $< 2>/dev/null \
...
#
# Just check modversions info, and touch .checkdep-stamp in TOPDIR if
# 'make dep' has to be rerun to rebuild it.
#
checkdep: $(patsubst %,_scdep_%,$(ALL_SUB_DIRS)) \
          $(addprefix $(MODINCL)/,$(export-objs:.o=.checkver))

ifdef ALL_SUB_DIRS
$(patsubst %,_scdep_%,$(ALL_SUB_DIRS)): dummy
	$(MAKE) -C $(patsubst _scdep_%,%,$@) _CHECKDEP_ONLY=1 checkdep
endif

$(MODINCL)/%.checkver: %.c dummy
	@if [ ! -r $(MODINCL)/$*.stamp -o $(MODINCL)/$*.stamp -ot $< ]; then \
		echo '$(CC) $(CFLAGS) $(EXTRA_CFLAGS_nostdinc) -E -D__GENKSYMS__ $<'; \
		echo '| $(GENKSYMS) $(genksyms_smp_prefix) -k $(VERSION).$(PATCHLEVEL).$(SUBLEVEL) > $@.tmp'; \
		$(CC) $(CFLAGS) $(EXTRA_CFLAGS_nostdinc) -E -D__GENKSYMS__ $< 2>/dev/null \
		| $(GENKSYMS) $(genksyms_smp_prefix) -k $(VERSION).$(PATCHLEVEL).$(SUBLEVEL) > $@.tmp; \
		if [ -r $(@:.checkver=.ver) ] && cmp -s $(@:.checkver=.ver) $@.tmp; then \
			touch $(MODINCL)/$*.stamp; \
		else \
			echo $< >> $(TOPDIR)/.checkdep-stamp; \
		fi; \
		rm -f $@.tmp; \
	fi
