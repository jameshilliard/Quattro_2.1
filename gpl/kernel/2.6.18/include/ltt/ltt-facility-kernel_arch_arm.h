...
enum lttng_syscall_name {
	...
	LTTNG_ltt_trace_generic = 322,
	LTTNG_ltt_register_generic = 323,
	...
	list_for_each_entry_rcu(trace, &ltt_traces.head, list) {
		...
		*to_base = *to = *len = 0;
		...
		ltt_write_event_header(trace, channel, buffer,
			ltt_facility_kernel_arch_2B44F74C, event_kernel_arch_syscall_entry,
		...
		void)
{
	...
	list_for_each_entry_rcu(trace, &ltt_traces.head, list) {
		...
		*to_base = *to = *len = 0;
		...
		ltt_write_event_header(trace, channel, buffer,
			ltt_facility_kernel_arch_2B44F74C, event_kernel_arch_syscall_exit,
			reserve_size, before_hdr_pad, tsc);
		*to_base += before_hdr_pad + after_hdr_pad + header_size;
