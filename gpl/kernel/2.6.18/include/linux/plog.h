...
// "[nopid] Bottom half start:%d pc=0x%08x"
struct _pl_bhre {
	int           num;
	unsigned long pc;
};
...
/* XXX: The fixup for parser is:
 * "Page Fault: pid:%d memory location:0x%08x pc=0x%08x preempt_count:%d"
 */
struct _pl_pfl {
	int pid;
	unsigned long addr;
...
#define PLOG_TYPE_FLUSH_CACHE 140
// "Cache Flush Routine:%d ra=0x%08x"
struct _pl_flush_c {
	int num;
	unsigned long ra;
};
/* 
 * 'num' is being populated depending upon what flush function 
 * had been called. The following enum can be referenced at the 
 * time of parsing e.g. 'num=2' implies 'local_r4k_flush_cache_range'
 * was called.
enum {
        f_bcm_local_inv_rac_all,
        f_local_r4k_flush_cache_all,
        f_local_r4k_flush_cache_range,
        f_brcm_r4k_flush_cache_range,
        f_local_r4k_flush_cache_mm,
        f_local_r4k_flush_cache_page,
        f_local_r4k_flush_icache_range,
        f_local_r4k_flush_icache_page,
        f_local_r4k_flush_cache_sigtramp,
        f_r4k_flush_dcache_all,
        f_r4k_flush_icache_all
};
*/

#define PLOG_TYPE_DMA_CACHE 141
// "DMA Coherency Routine:%d addr:0x%08x size:%d ra=0x%08x"
struct _pl_dma_c {
	int num;
	unsigned long addr;
	unsigned long size;
...
/*
 * 'num' is being populated depending upon what cache function 
 * had been called. The following enum can be referenced at the 
 * time of parsing e.g. 'num=1' implies 'r4k_dma_cache_inv'
 * was called.
enum {
	f_r4k_dma_cache_wback_inv,
        f_r4k_dma_cache_inv
};
*/
...
#define PLOG_TYPE_SW_WDOG_KEEPALIVE 144
#define PLOG_TYPE_HW_WDOG_KEEPALIVE 145
...
#ifdef MODULE
// A module may be compiled with a development kernel and run with a production kernel 
// The production kernel does not have these generic_event interfaces 
// In drivers, we never compile out this interfece, we test at run time. 

extern void _plog_generic_event0(int event) __attribute__((weak)) ;
extern void _plog_generic_event(int event, int somedata) __attribute__((weak));
extern void _plog_generic_event2(int event, int data1, int data2) __attribute__((weak));
extern void _plog_generic_event3(int event, int data1, int data2, int data3) __attribute__((weak));
extern void _plog_generic_event4(int event, 
                                 int data1, 
                                 int data2, 
                                 int data3, 
                                 int data4) __attribute__((weak));

#define plog_generic_event0(x) \
	do { if ( _plog_generic_event0 != NULL) { _plog_generic_event0(x);} } while (0)

#define plog_generic_event(x,y) \
	do { if ( _plog_generic_event != NULL) { _plog_generic_event(x,y);}} while (0)

#define plog_generic_event2(x,y,z)\
	do { if ( _plog_generic_event_2 != NULL) { _ploggeneric_event2(x,y,z);}} while (0)
	
#define plog_generic_event3(x,y,z,w)\
	do { if ( _plog_generic_event3 != NULL) { _plog_generic_event3(x,y,z,w);}} while (0)

#define plog_generic_event4(x,y,z,w,v) \
	do { if ( _plog_generic_event_4 != NULL) { _plog_generic_event_4(x,y,z,w,v);}} while (0)
...
extern void _plog_enable(int enable);
...
extern void _plog_bh_enter(int num, unsigned long pc);
...
extern void _plog_diskstart_sata(struct request *rq);
extern void _plog_diskstop_sata(struct request *rq, int write);
...
extern void _plog_pagefault_ent(unsigned long addr, unsigned long pc);
extern void _plog_flush_cache(int num, unsigned long ra);
extern void _plog_dma_coh(int num, unsigned long addr, unsigned long size, unsigned long ra);
...
#define plog_diskstart_sata _plog_diskstart_sata
#define plog_diskstop_sata _plog_diskstop_sata
...
#define plog_pagefault_ent _plog_pagefault_ent
#define plog_flush_cache _plog_flush_cache
#define plog_dma_coh _plog_dma_coh
...
#define _plog_enable(enable) do { } while (0)
...
#define plog_bh_enter(x,y) do { } while (0)
...
#define plog_diskstart_sata(x) do { } while (0)
#define plog_diskstop_sata(x,y) do { } while (0)
...
#define plog_pagefault_ent(x,y) do { } while (0)
#define plog_flush_cache(x) do { } while (0)
#define plog_dma_coh(x,y,z) do { } while (0)
