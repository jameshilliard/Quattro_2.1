...
#ifdef CONFIG_TIVO_FBUFS
#include <linux/fastbufs.h>
extern context_fbuf *find_cntxt(int pid);
#endif
...
#else
        E(PROC_TGID_CMDLINE,   "cmdline", S_IFREG|S_IRUGO|S_IWUSR),
#endif    
#ifdef CONFIG_TIVO_FBUFS
	E(PROC_TGID_EXEC_CNTXT,"exec_cntxt", S_IFREG|S_IRUGO|S_IWUSR),
#endif
...
#ifdef CONFIG_TIVO_FBUFS
	E(PROC_TID_EXEC_CNTXT, "exec_cntxt", S_IFREG|S_IRUGO),
#endif
...
#ifdef CONFIG_TIVO_FBUFS
static int proc_pid_exec_cntxt(struct task_struct *task, char * buffer)
{
	int res = 0;
        context_fbuf * cntxt; 
        cntxt = find_cntxt(task->pid);
        if (cntxt)
        {
                res = strlen(cntxt->name);
                res = (res> 63) ? 63 : res;
                strncpy(buffer,cntxt->name,res);
                buffer[res+1] = '\0';
        }    
        return res;
}
...
static int proc_pid_cmdline(struct task_struct *task, char * buffer)
{
    ...
    if (*(task->cmdline))
    {
        // return the magic string installed by previous write to /proc/XX/cmdline
        // the string could fill the entire buffer without zero-termination
        strncpy(buffer,task->cmdline,sizeof(task->cmdline)); 
        res=strnlen(buffer, sizeof(task->cmdline));
        goto out;
    }    
#endif
	unsigned int len;
...
#ifdef CONFIG_TIVO
// write to /proc/XX/cmdline
static ssize_t proc_pid_write_cmdline(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
    struct task_struct *task;

    if (!capable(CAP_SYS_RESOURCE)) return -EPERM; 
    if (!(task = get_proc_task(file->f_dentry->d_inode))) return -ESRCH;
    int n = count;  
    if (n > sizeof(task->cmdline)-1) n = sizeof(task->cmdline)-1;
    if (copy_from_user(task->cmdline, buf, n)) return -EFAULT;
    task->cmdline[n]=0;
    for (n=0; task->cmdline[n]>=' ' && task->cmdline[n]<='~'; n++);  
    task->cmdline[n]=0;
    put_task_struct(task);
    return count; // don't come back with more
}
#endif
#ifdef CONFIG_TIVO_FBUFS
static ssize_t proc_exec_cntxt_read(struct file * file, char __user * buf,
			  size_t count, loff_t *ppos)
{
	...
	if (!task)
        ...
        length = strlen(cntxt->name);

	if (__ppos >= length)
		return 0;
	if (count > length - __ppos)
		count = length - __ppos;
        if ( copy_to_user(buf, cntxt->name, count))
		return -EFAULT;
	*ppos = __ppos + count;
	put_task_struct(task);
out_no_task:
...
#ifdef CONFIG_TIVO
static struct file_operations proc_cmdline_file_operations = 
{ 
        .read = proc_info_read, 
        .write = proc_pid_write_cmdline 
};
#endif
#ifdef CONFIG_TIVO_FBUFS
static struct file_operations proc_exec_cntxt_file_operations = 
{ 
        .read = proc_exec_cntxt_read, 
};
#endif
...
#else            
			inode->i_fop = &proc_cmdline_file_operations;
			ei->op.proc_read = proc_pid_cmdline;
#endif      
			break;
#ifdef CONFIG_TIVO_FBUFS
		case PROC_TGID_EXEC_CNTXT:
		case PROC_TID_EXEC_CNTXT:
			inode->i_fop = &proc_exec_cntxt_file_operations;
			ei->op.proc_read = proc_pid_exec_cntxt;
#endif      
			break;
		case PROC_TID_STATM:
