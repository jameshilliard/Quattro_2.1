...
#else
    /* TiVo calls this from setup.c to configure bmem */
    int __init bmem_setup(char *str)
#endif
{
	...
	unsigned long start, struct page **page)
{
	...
	pfn = pte_pfn(*pte);
#if defined(CONFIG_TIVO) && defined(CONFIG_CONTIGMEM)
{
	extern int contigmem_check(unsigned long addr, unsigned long size);

	/*
	 *contigmem also needs to be handled this way else
	 * __get_user_pages will fail
	 */
	if ((bmem_find_region(pfn << PAGE_SHIFT, PAGE_SIZE) < 0) &&
	    !contigmem_check(pfn << PAGE_SHIFT, PAGE_SIZE))
		goto out;
}
#else
