        ...
        struct task_struct *tsk,int user_stack)
{
    ...
    if ( user_stack && (tsk != current) ){
        frame_ibuf = kmalloc(FRAME_IBUF_SZ,GFP_KERNEL);
        if ( !frame_ibuf ) {
            retval = -ENOMEM;
            goto out;
        }
        if ( !access_process_vm(tsk, (unsigned long)(pc-1), frame_ibuf, FRAME_IBUF_SZ, 0) ){
            retval = -EFAULT;
            goto out;
        }
    ...
    while (1) {
        ...
        if ( user_stack && (tsk != current)){
            if( (unsigned int)pc < tpc || (unsigned int)pc >= (tpc + FRAME_IBUF_SZ)){
                if ( !access_process_vm(tsk, (unsigned int)pc, frame_ibuf, FRAME_IBUF_SZ, 0) ){
                    retval = -EFAULT;
                    goto out;
                }
                tpc = (unsigned int)pc;
        ...
        if (inst == 0x03e00008) { // jr ra
            ...
            *pra = (unsigned int *)0xffffffff;
            ...
            if (ra_pos == saved_ra_pos)
                ...
                if ( user_stack && (tsk != current) ){
                    if ( !access_process_vm(tsk, (unsigned int)ra_pos, &ra, 4, 0) ){
                        retval = -EFAULT;
                        goto out;
                    }
                }
                else{
                    int ra_int; /* Required because __get_user assigns ra_int = *ra_pos */
                    if (__get_user(ra_int, ra_pos)){
                        retval = -EFAULT;
                        goto out;
                    }
                    ra = (unsigned int*)ra_int;
                }
            }
        }
            ...
            if (fp_pos == saved_fp_pos)
                ...
                if ( user_stack && (tsk != current) ){
                    if ( !access_process_vm(tsk, (unsigned int)fp_pos, &fp, 4, 0) ){
                        retval = -EFAULT;
                        goto out;
                    }
                }
                else {
                    int fp_int; /* Required because __get_user assigns fp_int = *fp_pos */
                    if (__get_user(fp_int, fp_pos)){
                        retval = -EFAULT;
                        goto out;
                    }
                    fp = (unsigned int*)fp_int;
                }
            }
        }
        ...
        if ( user_stack && (tsk != current)){
            if( (unsigned int)(pc-1) < tpc || (unsigned int)(pc-1) >= (tpc + FRAME_IBUF_SZ)){
                if ( !access_process_vm(tsk, (unsigned int)(pc-1), frame_ibuf, FRAME_IBUF_SZ, 0) ){
                    retval = -EFAULT;
                    goto out;
                }
                tpc = (unsigned int)(pc-1);
    ...
    if ( user_stack && (tsk != current) ){
        if (!access_process_vm(tsk, (unsigned int)(pc+1), &inst, 4, 0)){
            retval = -EFAULT;
            goto out;
        }
    }
