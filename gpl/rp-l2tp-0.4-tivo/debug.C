...
/* Big bang is when the universe started... set by first call to db */
...
char const *l2tp_debug_session_to_str(l2tp_session *session)
{
	...
	sprintf(buf, "(%d/%d, %d/%d)", (int) session->tunnel->my_id, (int) session->tunnel->assigned_id, (int) session->my_id, (int) session->assigned_id);
	return buf;
}
...
char const * l2tp_debug_describe_dgram(l2tp_dgram const *dgram)
{
	...
	{
		// Control datagram
		snprintf(buf, sizeof(buf), "type=%s, tid=%d, sid=%d, Nr=%d, Ns=%d", l2tp_debug_message_type_to_str(dgram->msg_type), (int) dgram->tid, (int) dgram->sid, (int) dgram->Nr, (int) dgram->Ns);
	}
	else
	{
		snprintf(buf, sizeof(buf), "data message tid=%d sid=%d payload_len=%d", (int) dgram->tid, (int) dgram->sid, (int) dgram->payload_len);
	}
	return buf;
}
