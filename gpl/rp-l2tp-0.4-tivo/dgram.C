...
/*
static void describe_pulled_avp(uint16_t vendor, uint16_t type, uint16_t len, int hidden, int mandatory, void *val)
{
...
int l2tp_dgram_validate_avp(uint16_t vendor, uint16_t type, uint16_t len, int mandatory)
{
	if (vendor != VENDOR_IETF)
	{
		if (mandatory)
		{
			l2tp_set_errmsg("Unknown mandatory AVP (vendor %d, type %d)", (int) vendor, (int) type);
			return 0;
		}
		// I suppose...
		return 1;
	...
	if (type > HIGHEST_AVP)
	{
		if (mandatory)
		{
			l2tp_set_errmsg("Unknown mandatory AVP of type %d", (int) type);
			return 0;
		}
		return 1;
	}
...
l2tp_dgram *l2tp_dgram_new(size_t len)
{
	...
	{
		l2tp_set_errmsg("dgram_new: cannot allocate datagram with payload length %d", (int) len);
		return NULL;
	}
	...
	{
		dgram = (l2tp_dgram *)malloc(sizeof(l2tp_dgram));
	}
	if (!dgram)
	{
...
l2tp_dgram *l2tp_dgram_take_from_wire(struct sockaddr_in *from)
{
	...
	// Limit iterations before bailing back to select look.  Otherwise, we have a nice DoS possibility
	int iters = 5;
	...
	while(1)
	{
		...
		if (!ses || tid != cache_tid || sid != cache_sid)
		{
			...
			if (!ses)
			{
				l2tp_set_errmsg("Unknown session %d in tunnel %d", (int) sid, (int) tid);
				continue;
			}
			cache_tid = tid;
			cache_sid = sid;
		...
		handle_ppp_frame(ses, payload, framelen);
	}
	...
	DBG(l2tp_db(DBG_XMIT_RCV, "dgram_take_from_wire() -> %s\n", l2tp_debug_describe_dgram(dgram)));
	return dgram;
}
...
int l2tp_dgram_send_to_wire(l2tp_dgram const *dgram, struct sockaddr_in const *to)
{
	...
	DBG(l2tp_db(DBG_XMIT_RCV, "dgram_send_to_wire() -> %s\n", l2tp_debug_describe_dgram(dgram)));
	buf[0] = dgram->bits;
	buf[1] = dgram->version;
...
*  padding (should we?)
***********************************************************************/
static void dgram_do_hide(uint16_t type, uint16_t len, unsigned char const *value, unsigned char const *secret, size_t secret_len, unsigned char const *random_data, size_t random_len, unsigned char *output)
{
...
***********************************************************************/
static unsigned char *dgram_do_unhide(uint16_t type, uint16_t *len, unsigned char *value, unsigned char const *secret, size_t secret_len, unsigned char const *random_data, size_t random_len, unsigned char *output)
{
	struct MD5Context ctx;
	unsigned char t[2];
	unsigned char digest[16];
	...
	if (tmplen > *len-8)
	{
		l2tp_set_errmsg("Hidden length %d too long in AVP of length %d", (int) tmplen, (int) *len);
		return NULL;
	}
...
int l2tp_dgram_add_avp(l2tp_dgram *dgram, l2tp_tunnel *tunnel, int mandatory, uint16_t len, uint16_t vendor, uint16_t type,const void *val)
{
	...
	if (tunnel && tunnel->peer && tunnel->peer->hide_avps && vendor == VENDOR_IETF && tunnel->peer->secret_len && len <= 1021 - 6 && avp_table[type].can_hide)
	{
		if (!dgram->last_random)
		{
			// Add a random vector
			if (dgram_add_random_vector_avp(dgram) < 0)
			{
				return -1;
			}
		}
		...
		dgram_do_hide(type, len, (const unsigned char *)val, (unsigned char *) tunnel->peer->secret, tunnel->peer->secret_len, random_data, random_len, hidden_buffer);
...
unsigned char *l2tp_dgram_pull_avp(l2tp_dgram *dgram, l2tp_tunnel *tunnel, int *mandatory, int *hidden, uint16_t *len, uint16_t *vendor, uint16_t *type, int *err)
{
	...
	*len = ((unsigned int) (dgram->data[dgram->cursor] & 3)) * 256 + dgram->data[dgram->cursor+1];
	...
	{
		l2tp_set_errmsg("Received AVP of length %d too long for rest of datagram", (int) *len);
		return NULL;
	}
	dgram->cursor += 2;
	...
	if (*hidden) {
		...
		if (*type <= HIGHEST_AVP)
		{
			...
			{
				l2tp_set_errmsg("AVP of type %s cannot be hidden, but H bit set", avp_table[*type].name);
				return NULL;
			}
		}
		...
		// Unhide value
		ans = dgram_do_unhide(*type, len, ans, (unsigned char *) tunnel->peer->secret, tunnel->peer->secret_len, random_data, random_len, val);
		if (!ans) return NULL;
	}
...
unsigned char *l2tp_dgram_search_avp(l2tp_dgram *dgram, l2tp_tunnel *tunnel, int *mandatory, int *hidden, uint16_t *len, uint16_t vendor, uint16_t type)
{
	...
	while(1)
	{
		...
		if (!val)
		{
			if (!err)
			{
				l2tp_set_errmsg("AVP of vendor/type (%d, %d) not found", (int) vendor, (int) type);
			}
			dgram->cursor = cursor;
			dgram->last_random = last_random;
			return NULL;
		}
		if (vendor == svend && type == stype)
		{
			break;
		}
	}
...
int l2tp_dgram_send_ppp_frame(l2tp_session *ses, unsigned char *buf, int len)
{
	...
	if (!tunnel || tunnel->state != TUNNEL_ESTABLISHED || ses->state != SESSION_ESTABLISHED)
	{
		return 0;
	}
