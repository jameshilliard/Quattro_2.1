...
// These functions are used recursively, so make declarations here
static void tunnel_handle_timeout(EventSelector *es, int fd, unsigned int flags, void *data);
static void tunnel_do_hello(EventSelector *es, int fd, unsigned int flags, void *data);
...
static void tunnel_set_state(l2tp_tunnel *tunnel, int state)
{
	...
	DBG(l2tp_db(DBG_TUNNEL, "tunnel(%s) state %s -> %s\n", l2tp_debug_tunnel_to_str(tunnel), state_names[tunnel->state], state_names[state]));
	tunnel->state = state;
}
...
l2tp_tunnel *l2tp_tunnel_find_by_my_id(uint16_t id)
{
	...
	candidate.my_id = id;
	return (l2tp_tunnel *)hash_find(&tunnels_by_my_id, &candidate);
}
...
static l2tp_tunnel *tunnel_find_bypeer(struct sockaddr_in addr)
{
	...
	return (l2tp_tunnel *)hash_find(&tunnels_by_peer_address, &candidate);
}
...
static char const *tunnel_flow_control_stats(l2tp_tunnel *tunnel)
{
	...
	snprintf(buf, sizeof(buf), "rws=%d cwnd=%d ssthresh=%d outstanding=%d", (int) tunnel->peer_rws, tunnel->cwnd, tunnel->ssthresh, tunnel_outstanding_frames(tunnel));
	return buf;
}
...
static uint16_t tunnel_make_tid(void)
{
	...
	for(;;)
	{
		...
		if (id)
		{
			if (!l2tp_tunnel_find_by_my_id(id))
			{
				return id;
			}
		}
...
static void tunnel_enqueue_dgram(l2tp_tunnel *tunnel, l2tp_dgram *dgram)
{
	...
	dgram->Ns = tunnel->Ns;
	tunnel->Ns++;
	DBG(l2tp_db(DBG_FLOW, "tunnel_enqueue_dgram(%s, %s) %s\n", l2tp_debug_tunnel_to_str(tunnel), l2tp_debug_message_type_to_str(dgram->msg_type), tunnel_flow_control_stats(tunnel)));
...
static void tunnel_free(l2tp_tunnel *tunnel)
{
	...
	for (ses = (l2tp_session *)hash_start(&tunnel->sessions_by_my_id, &cursor); ses ; ses = (l2tp_session *)hash_next(&tunnel->sessions_by_my_id, &cursor))
	{
		l2tp_session_free(ses, "Tunnel closing down");
	}
	if (tunnel->hello_handler)
	{
		Event_DelHandler(es, tunnel->hello_handler);
	}
	...
	{
		Event_DelHandler(es, tunnel->ack_handler);
	}
...
static void tunnel_xmit_queued_messages(l2tp_tunnel *tunnel)
{
	...
	{
		tunnel->timeout_handler = Event_AddTimerHandler(tunnel->es, t, tunnel_handle_timeout, tunnel);
	}
}
...
static void tunnel_handle_timeout(EventSelector *es, int fd, unsigned int flags, void *data)
{
	...
	{
		l2tp_set_errmsg("Too many retransmissions on tunnel (%s); closing down", l2tp_debug_tunnel_to_str(tunnel));
		// Close tunnel...
		tunnel_free(tunnel);
		return;
	}
...
static unsigned int tunnel_compute_peerhash(void *data)
{
	return (unsigned int) (((l2tp_tunnel *)data)->peer_addr.sin_addr.s_addr);
}

static int tunnel_compare_peer(void *d1, void *d2)
{
	return (((l2tp_tunnel *)d1)->peer_addr.sin_addr.s_addr) != (((l2tp_tunnel *)d2)->peer_addr.sin_addr.s_addr);
}
...
*  My tunnel ID
***********************************************************************/
static unsigned int tunnel_compute_hash_my_id(void *data)
{
...
***********************************************************************/
static int tunnel_compare_my_id(void *item1, void *item2)
{
...
void l2tp_tunnel_stop_tunnel(l2tp_tunnel *tunnel, char const *reason)
{
	...
	if (tunnel->state != TUNNEL_RECEIVED_STOP_CCN && tunnel->state != TUNNEL_SENT_STOP_CCN)
	{
...
void l2tp_tunnel_stop_all(char const *reason)
{
	...
	// Send StopCCN on all tunnels except those which are scheduled for destruction
	for (tunnel = (l2tp_tunnel *)hash_start(&tunnels_by_my_id, &cursor); tunnel; tunnel = (l2tp_tunnel *)hash_next(&tunnels_by_my_id, &cursor))
	{
...
***********************************************************************/
static void tunnel_cleanup(void *data)
{
...
{
	l2tp_tunnel *tunnel = (l2tp_tunnel *)malloc(sizeof(l2tp_tunnel));
	if (!tunnel)
	{
		return NULL;
	}
...
static int tunnel_send_SCCRQ(l2tp_tunnel *tunnel)
{
	...
	// Add the AVP's
	// HACK!  Cisco equipment cannot handle hidden AVP's in SCCRQ. So we temporarily disable AVP hiding
	...
	if (tunnel->peer->secret_len)
	{
		...
		l2tp_auth_gen_response(MESSAGE_SCCRP, tunnel->peer->secret, challenge, sizeof(challenge), tunnel->expected_response);
	}
...
static int tunnel_set_params(l2tp_tunnel *tunnel, l2tp_dgram *dgram)
{
	...
	// Get host name
	val = l2tp_dgram_search_avp(dgram, tunnel, &mandatory, &hidden, &len, VENDOR_IETF, AVP_HOST_NAME);
	if (!val)
	{
	...
	if (!tunnel->peer)
	{
		l2tp_set_errmsg("Peer %s is not authorized to create a tunnel", inet_ntoa(tunnel->peer_addr.sin_addr));
	...
	while(1)
	{
		...
		if (vendor != VENDOR_IETF)
		{
			...
			tunnel_send_StopCCN(tunnel, RESULT_GENERAL_ERROR, ERROR_UNKNOWN_AVP_WITH_M_BIT, "Unknown mandatory attribute (vendor=%d, type=%d) in %s", (int) vendor, (int) type, l2tp_debug_avp_type_to_str(dgram->msg_type));
			return -1;
		}
		...
		switch(type)
		{
			...
			case AVP_CHALLENGE:
				if (tunnel->peer->secret_len)
				{
					l2tp_auth_gen_response( ((dgram->msg_type == MESSAGE_SCCRQ) ? MESSAGE_SCCRP : MESSAGE_SCCCN), tunnel->peer->secret, val, len, tunnel->response);
				}
				break;
...
{
	l2tp_dgram *dgram = l2tp_dgram_new_control(MESSAGE_ZLB, tunnel->assigned_id, 0);
	if (!dgram)
	{
...
***********************************************************************/
static void tunnel_do_ack(EventSelector *es, int fd, unsigned int flags, void *data)
{
	l2tp_tunnel *tunnel = (l2tp_tunnel *) data;
...
static void tunnel_schedule_ack(l2tp_tunnel *tunnel)
{
	...
	t.tv_sec = 0;
	t.tv_usec = 100000;
	tunnel->ack_handler = Event_AddTimerHandler(tunnel->es,	t, tunnel_do_ack, tunnel);
}
...
static void tunnel_setup_hello(l2tp_tunnel *tunnel)
{
	...
	tunnel->hello_handler = Event_AddTimerHandler(tunnel->es, t, tunnel_do_hello, tunnel);
}
...
static void tunnel_handle_SCCRP(l2tp_tunnel *tunnel, l2tp_dgram *dgram)
{
	...
	if (tunnel_set_params(tunnel, dgram) < 0)
	{
		return;
	}
...
{
	// TODO: Send CDN (we don't take incoming calls)
	return;
}

/**********************************************************************
* %FUNCTION: l2tp_tunnel_find_session
* %ARGUMENTS:
*  sid -- session ID
* %RETURNS:
...
l2tp_session *l2tp_tunnel_find_session(l2tp_tunnel *tunnel, uint16_t sid)
{
	...
	candidate.my_id = sid;
	return (l2tp_session *)hash_find(&tunnel->sessions_by_my_id, &candidate);
}
...
static void tunnel_process_received_datagram(l2tp_tunnel *tunnel, l2tp_dgram *dgram)
{
	...
	DBG(l2tp_db(DBG_TUNNEL, "tunnel_process_received_datagram(%s, %s)\n", l2tp_debug_tunnel_to_str(tunnel), l2tp_debug_message_type_to_str(dgram->msg_type)));
	switch (dgram->msg_type)
	{
		...
		case MESSAGE_CDN:
			...
			if (!ses)
			{
				l2tp_set_errmsg("Session-related message for unknown session %d", (int) dgram->sid);
				return;
			}
	}
...
*  Deallocates all tunnel state
***********************************************************************/
static void tunnel_finally_cleanup(EventSelector *es, int fd, unsigned int flags, void *data)
{
	l2tp_tunnel *tunnel = (l2tp_tunnel *) data;
...
static void tunnel_schedule_destruction(l2tp_tunnel *tunnel)
{
	...
	tunnel->hello_handler = Event_AddTimerHandler(tunnel->es, t, tunnel_finally_cleanup, tunnel);
...
void l2tp_tunnel_handle_received_control_datagram(l2tp_dgram *dgram, EventSelector *es, struct sockaddr_in *from)
{
	...
	// If it's SCCRQ, then ignore it (we don't accept incoming requests)
	...
	if (!tunnel)
	{
		...
		l2tp_set_errmsg("Invalid control message - unknown tunnel ID %d", (int) dgram->tid);
		return;
	}
	...
	if (tunnel->state != TUNNEL_RECEIVED_STOP_CCN && tunnel->state != TUNNEL_SENT_STOP_CCN && tunnel->hello_handler != NULL)
	{
		struct timeval t;
		t.tv_sec = 60;
		t.tv_usec = 0;
...
l2tp_tunnel *l2tp_tunnel_find_for_peer(l2tp_peer *peer, EventSelector *es)
{
	...
	if (tunnel)
	{
		if (tunnel->state == TUNNEL_WAIT_CTL_REPLY || tunnel->state == TUNNEL_WAIT_CTL_CONN || tunnel->state == TUNNEL_ESTABLISHED)
		{
			return tunnel;
		}
	}
...
{
	return (l2tp_session *)hash_next(&tunnel->sessions_by_my_id, cursor);
}
...
{
	return (l2tp_session *)hash_start(&tunnel->sessions_by_my_id, cursor);
}
...
{
	return (l2tp_tunnel *)hash_next(&tunnels_by_my_id, cursor);
}
...
{
	return (l2tp_tunnel *)hash_start(&tunnels_by_my_id, cursor);
}

/**********************************************************************
* %FUNCTION: l2tp_tunnel_init
* %ARGUMENTS:
*  es -- event selector
* %RETURNS:
*  Nothing
...
{
	hash_init(&tunnels_by_my_id, offsetof(l2tp_tunnel, hash_by_my_id), tunnel_compute_hash_my_id, tunnel_compare_my_id);
	hash_init(&tunnels_by_peer_address, offsetof(l2tp_tunnel, hash_by_peer), tunnel_compute_peerhash, tunnel_compare_peer);
