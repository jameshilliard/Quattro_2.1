  ...
  case TF_show_proc|TF_loose_tasks:    // H option
    while(readproc(ptp,&buf)){
      ...
      while(readtask(ptp,&buf,&buf2)){
        ...
        // we use custom thread command lines
        if(buf2.cmdline) free((void*)*buf2.cmdline);
#endif
      }
      if(buf.cmdline) free((void*)*buf.cmdline); // ought to reuse
      if(buf.environ) free((void*)*buf.environ); // ought to reuse
...
#else
        // we use custom thread command lines allocated per task
        while(readtask(ptp,&buf,&buf2)) {
          show_one_proc(&buf2, task_format_list);
          if(buf2.cmdline) free((void*)*buf2.cmdline);
        }
...
#else
      // TiVo customization to show thread command line
      while(readtask(ptp,*p,&buf2)) {
        show_one_proc(&buf2, task_format_list);
        if(buf2.cmdline) free((void*)*buf2.cmdline);
      }
#endif
