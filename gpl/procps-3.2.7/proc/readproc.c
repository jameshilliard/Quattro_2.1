...
#else
    if ((flags & PROC_FILLCOM) || (flags & PROC_FILLARG))	/* read+parse /proc/p/task/t/cmdline */
        t->cmdline = file2strvec(path, "cmdline");
    else
        t->cmdline = NULL;
#endif
...
#ifdef TIVO
    if (p->cmdline != NULL) {
      // clone the proc command line
      char *buf, **ptrs, **newptrs;
      int align, tot, strcount;
      ...
      strcount = 0;
      tot = 0;
      while (ptrs[strcount] != NULL) {
        tot += strlen(ptrs[strcount]) + 1;
        ++strcount;
      }

      align = (sizeof(char*)-1) - ((tot + sizeof(char*)-1) & (sizeof(char*)-1));
      buf = malloc((strcount + 1) * sizeof(char *) + tot + align);
      // ignore allocation failures like the rest of the code does
      t->cmdline = newptrs = (char **)(buf + tot + align);

      strcount = 0;
      while (ptrs[strcount] != NULL) {
        *newptrs++ = buf;
	strcpy(buf, ptrs[strcount]);
        buf += strlen(ptrs[strcount]) + 1;
        ++strcount;
      }
      *newptrs++ = NULL;
    }
